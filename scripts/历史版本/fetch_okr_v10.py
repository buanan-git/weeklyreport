...
def is_meaningless_line(line):
    ...
    # 检测纯tab（连续5个及以上）
    if stripped.replace('\t', '') == '' and len(stripped) >= 5:
        return True, 'tabs'
    
    # 检测纯空格（连续50个及以上）
    if stripped.replace(' ', '') == '' and len(stripped) >= 50:
        return True, 'spaces'
    
    # 检测其他无意义格式符（连续50个及以上，只剩不可见字符）
    visible_chars = ''.join(c for c in stripped if c.isprintable() and not c.isspace())
    if visible_chars == '' and len(stripped) >= 50:
        return True, 'format'
    
    return False, None

def truncate_at_meaningless_content(full_text, max_consecutive=5):
    """
    当检测到以下情况时，截取有效内容部分：
    - 连续5个空行
    - 连续5个tab键行
    - 连续50个空格
    - 连续50个其他无意义格式符
    返回截取后的文本（不包含已检测到的无效内容）
    """
    lines = full_text.split('\n')
    result_lines = []
    meaningless_count = 0
    last_meaningless_type = None
    
    for line in lines:
        is_meaningless, mtype = is_meaningless_line(line)
        
        if is_meaningless:
            meaningless_count += 1
            if meaningless_count > max_consecutive:
                # 超过阈值，停止截取（不添加这行，直接结束）
                print(f"  [V10优化] 检测到连续{max_consecutive}个无意义行({mtype})，停止抓取")
                break
            # 小于阈值的无意义行也不添加到结果
            last_meaningless_type = mtype
        else:
            # 遇到有效内容，重置计数器
...
def parse_weekly_report_structure_v10(full_text):
    ...
    full_text = truncate_at_meaningless_content(full_text, max_consecutive=5)
    truncated_length = len(full_text)
    if truncated_length < original_length:
        print(f"  [V10优化] 已截取有效内容: {original_length} -> {truncated_length}字符")
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for i, staff_id in enumerate(STAFF_IDS, 1):
            ...
            result = await fetch_single_staff_v10(page, staff_id, last_week, f"下属{i}", i, len(STAFF_IDS))
        ...
        if not any(r['status'] == 'login_required' for r in all_results):
            ...
            print(f"{'='*60}")
            await asyncio.sleep(1.5)
            print(f"fetch_single_staff_v10参数：{page},{MY_ID},{last_week}")

            r1 = await fetch_single_staff_v10(page, MY_ID, last_week, "本人-上周", 1, 2)
            ...
            if r1['status'] != 'login_required':
                await asyncio.sleep(2)
                print(f"fetch_single_staff_v10参数：{page},{MY_ID},{week_before}")
                r2 = await fetch_single_staff_v10(page, MY_ID, week_before, "本人-上上周", 2, 2)
