...
async def fill_okr_section(page, part2_data):
    ...
    try:
        ...
        table_info = await page.evaluate("""() => {
            ...
            for (const el of allElements) {
                if (el.textContent && el.textContent.includes('卜安安的OKR')) {
                    ...
                    for (let i = 0; i < 5; i++) {
                        ...
                        // 检查这个容器是否包含表格结构
                        const hasTableStructure = parent.querySelector('table') || 
                                                  parent.querySelector('[class*="okr-card"]') ||
                                                  parent.querySelector('[class*="objective"]') ||
                                                  parent.querySelector('[class*="key-result"]');
            ...
            // 3. 在表格容器内查找所有"本周工作"文本
            const allSpans = tableContainer.querySelectorAll('span, div');
                ...
                if (span.textContent && span.textContent.trim() === '本周工作') {
                    ...
                    for (let i = 0; i < 5; i++) {
                        ...
                        if (editable && !processedInputs.has(editable)) {
                            // 验证这个输入区域是否真的在表格内（通过检查父元素）
                            if (tableContainer.contains(editable)) {
        ...
        if table_info.get('inputCount') != total_krs:
            print(f"  [⚠️] 表格内输入区域数量({table_info.get('inputCount')})与KR数量({total_krs})不匹配")
            print(f"  预期: {total_krs} 个, 实际: {table_info.get('inputCount')} 个")
            
            if table_info.get('inputCount') > total_krs:
                print(f"  多出了 {table_info.get('inputCount') - total_krs} 个输入区域")
        ...
        await asyncio.sleep(1)
        input("\n  请确认：1.蓝色OKR进度标题 2.红色表格区域 3.绿色输入区域（只应在表格内，且坐标为正），按Enter继续...")
        
        # ==================== 第二步：获取并显示每个输入区域的位置 ====================
        print("\n" + "-"*60)
        print("  [步骤2] 获取输入区域精确位置")
        print("-"*60)
        ...
        input_positions = await page.evaluate("""() => {
            ...
            positions.forEach((pos, idx) => {
                pos.displayIndex = idx + 1;
            });
            
            return positions;
        }""")
        
        print(f"  获取到 {len(input_positions)} 个有效输入区域")
        ...
        # 检查是否有负坐标
        negative_positions = [p for p in input_positions if p['y'] < 0 or p['x'] < 0]
        if negative_positions:
            print(f"  [⚠️] 发现 {len(negative_positions)} 个负坐标输入区域，已被过滤")
        ...
        for obj_idx, obj in enumerate(objectives):
            ...
            for kr_idx, json_kr in enumerate(json_krs):
                ...
                if input_index < len(input_positions):
                    ...
                    print(f"      原始坐标: ({pos['rawX']:.0f}, {pos['rawY']:.0f})")
                    ...
                    # 滚动到位置
                    await page.evaluate(f"window.scrollTo(0, {pos['rawY'] - 200})")
                    await asyncio.sleep(0.5)
