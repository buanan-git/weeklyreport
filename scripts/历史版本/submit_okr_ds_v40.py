...
async def fill_okr_section(page, part2_data):
    """填写第二部分：OKR进度 - 基于第一部分经验版"""
    
    print("\n" + "="*80)
    print("[填写] 第二部分：OKR进度（强化版）")
    print("="*80)
    ...
    try:
        ...
        # 定位并高亮OKR区域
        region_info = await page.evaluate("""() => {
            const result = {
                success: false,
                message: '',
                oCount: 0,
            ...
            // 2. 找到包含"卜安安的OKR"的容器
            const allElements = document.querySelectorAll('div, section');
            let okrContainer = null;
            
            for (const el of allElements) {
                if (el.textContent && el.textContent.includes('卜安安的OKR')) {
                    okrContainer = el.closest('div[class*="container"], div[class*="card"], section') || el.parentElement;
                    if (okrContainer) {
                        okrContainer.style.border = '3px solid red';
                        okrContainer.style.backgroundColor = 'rgba(255, 0, 0, 0.05)';
                    }
                    break;
                }
            }
            ...
            if (!okrContainer) {
                result.message = '未找到OKR容器';
                return result;
            }
            
            // 3. 查找所有Objective容器
            const objectiveBlocks = okrContainer.querySelectorAll('[class*="objective"], [class*="Objective"]');
            result.oCount = objectiveBlocks.length;
            
            objectiveBlocks.forEach((block, idx) => {
                block.style.border = '2px solid orange';
                block.style.backgroundColor = 'rgba(255, 165, 0, 0.1)';
            ...
            // 4. 查找所有KR容器
            const krBlocks = okrContainer.querySelectorAll('[class*="key-result"], [class*="KeyResult"]');
            result.krCount = krBlocks.length;
            
            krBlocks.forEach((block, idx) => {
                block.style.border = '2px solid purple';
                block.style.backgroundColor = 'rgba(128, 0, 128, 0.1)';
            ...
            // 5. 查找所有"本周工作"输入区域
            const weeklyWorkElements = [];
            const allSpans = okrContainer.querySelectorAll('span');
            ...
            allSpans.forEach((span) => {
                if (span.textContent && span.textContent.includes('本周工作')) {
                    ...
                    for (let i = 0; i < 5; i++) {
                        ...
                        // 在容器内查找可编辑区域
                        const editable = container.querySelector('[contenteditable="true"], .mp-paragraph-wrapper, [class*="input"], [class*="editor"]');
                        if (editable) {
                            weeklyWorkElements.push({
                                span: span,
                                container: container,
                                selector: editable
                            });
                            
                            editable.style.border = '2px solid green';
                            ...
                            marker.style.zIndex = '10000';
                            marker.style.fontSize = '12px';
                            marker.textContent = `输入${weeklyWorkElements.length + 1}`;
                            document.body.appendChild(marker);
            ...
            result.inputCount = weeklyWorkElements.length;
            result.success = true;
            result.message = '成功定位OKR区域';
            return result;
        }""")
        ...
        if not region_info.get('success'):
            print(f"  [X] {region_info.get('message')}")
            return False
        
        print(f"  [OK] {region_info.get('message')}")
        print(f"  找到O元素: {region_info.get('oCount', 0)} 个")
        print(f"  找到KR元素: {region_info.get('krCount', 0)} 个")
        print(f"  找到本周工作输入区域: {region_info.get('inputCount', 0)} 个")
        ...
        # 获取所有输入区域的位置信息
        input_positions = await page.evaluate("""() => {
            const positions = [];
            const inputs = document.querySelectorAll('[style*="border: 2px solid green"]');
            ...
            inputs.forEach((input, index) => {
                try {
                    ...
                    if (rect.width > 0 && rect.height > 0) {
                        ...
                        marker.className = 'debug-marker';
                        marker.style.position = 'fixed';
                        marker.style.left = (rect.left + 10) + 'px';
        ...
        print(f"  找到 {len(input_positions)} 个可用的输入区域")
        
        if len(input_positions) != total_krs:
            print(f"  [⚠️] 输入区域数量({len(input_positions)})与KR数量({total_krs})不匹配")
        
        input("\n  请确认黄色点击标记的位置，按Enter开始填写...")
        ...
        for obj_idx, obj in enumerate(objectives):
            ...
            for kr_idx, kr in enumerate(key_results):
                ...
                if kr_counter < len(input_positions):
                    pos = input_positions[kr_counter]
