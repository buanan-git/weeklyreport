...
async def run_fetch_script():
    ...
    # 确保脚本可执行（非Windows系统）
    if not ensure_executable(FETCH_SCRIPT):
        log_warning(f"无法设置fetch脚本执行权限: {FETCH_SCRIPT}")
    ...
    try:
        # 使用跨平台安全的子进程运行
        result = safe_subprocess_run(cmd, cwd=SCRIPT_DIR, timeout=300)  # 5分钟超时
...
async def run_llm_optimization():
    ...
    # 确保脚本可执行（非Windows系统）
    if not ensure_executable(LLMCHAT_SCRIPT):
        log_warning(f"无法设置llmchat脚本执行权限: {LLMCHAT_SCRIPT}")
    ...
    try:
        # 使用跨平台安全的子进程运行
        result = safe_subprocess_run(cmd, cwd=SCRIPT_DIR, timeout=600)  # 10分钟超时
...
async def run_submit_script():
    ...
    # 确保脚本可执行（非Windows系统）
    if not ensure_executable(SUBMIT_SCRIPT):
        log_warning(f"无法设置submit脚本执行权限: {SUBMIT_SCRIPT}")
