    ...
    try:
        ...
        table_info = await page.evaluate("""() => {
            const result = {
                ...
                inputCount: 0,
                inputElements: [],
                debugInputs: []  // 新增：用于记录所有找到的输入框的详细信息
            };
                    ...
                    for (let i = 0; i < 5; i++) {
                        ...
                        if (editable && !processedInputs.has(editable)) {
                            ...
                            const inputInfo = {
                                ...
                                parentClasses: editable.parentElement ? editable.parentElement.className : '无父元素',
                                containerInTable: tableContainer.contains(editable), // 是否在表格容器内
                                spanText: span.textContent // 触发识别的"本周工作"文本
                            };
                            
                            // 记录到调试数组
                            result.debugInputs.push(inputInfo);
        ...
        # 打印所有识别到的输入框详细信息（包括被过滤的）
        print("\n  [调试信息] 所有识别到的'本周工作'输入框:")
        debug_inputs = table_info.get('debugInputs', [])
        for i, inp in enumerate(debug_inputs):
            status = "✅ 已保留" if inp['rawY'] > 0 else "❌ 已过滤(负坐标)"
            print(f"\n    输入框 {i+1}: {status}")
            print(f"      触发文本: {inp.get('spanText', '')}")
            print(f"      坐标: ({inp['x']:.0f}, {inp['y']:.0f}), 原始Y={inp['rawY']:.0f}")
            print(f"      标签名: {inp['tagName']}")
            print(f"      类名: {inp['className'][:100]}")
            print(f"      父元素类: {inp['parentClasses'][:100]}")
            print(f"      在表格容器内: {inp.get('containerInTable', False)}")
        
        if table_info.get('inputCount') != total_krs:
            print(f"\n  [⚠️] 表格内可见输入区域数量({table_info.get('inputCount')})与KR数量({total_krs})不匹配")
