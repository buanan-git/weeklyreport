...
async def fill_by_javascript(page, label_text, content):
    ...
    try:
        result = await page.evaluate("""([labelText, content]) => {
            console.log('JS: 开始查找标签:', labelText);
            const findEl = (text) => {
                const w = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, null, false);
                let n; 
                while (n = w.nextNode()) {
                    if (n.textContent.trim().includes(text)) return n.parentElement;
                }
                return null;
            };
            const findInput = (el) => {
            ...
            if (input.tagName === 'TEXTAREA' || input.tagName === 'INPUT') {
                input.value = content;
                console.log('JS: 已设置value');
            } else {
                // 使用字符串分割代替正则，避免转义问题
                input.innerHTML = content.split('\n').join('<br>');
