...
async def fill_part3_metrics(page, part3_data):
    """填写第三部分：关键指标 - 基于成功经验版（自动模式）"""
    
    print("\n" + "="*80)
    print("[填写] 第三部分：关键指标（基于成功经验版 - 自动模式）")
    print("="*80)
...
async def auto_submit_and_confirm(page):
    ...
    # 方法1: 直接查找"确定"按钮并滚动到视图
    print("\n  方法1: 直接查找'确定'按钮并滚动到视图...")
    try:
        # 查找所有可能的确定按钮
        confirm_selectors = [
            '.mp-btn.mp-modal-enter-btn',  # 从日志中看到的类名
            'button:has-text("确定")',
            '.ant-btn-primary:has-text("确定")',
            '.erp-modal-footer button:has-text("确定")',
            '.mp-modal-footer button:has-text("确定")'
        ]
    ...
    if not confirm_clicked:
        ...
        result = await page.evaluate("""() => {
            // 查找确定按钮
            const findAndClickConfirm = () => {
                // 多种选择器
                const selectors = [
                    '.mp-btn.mp-modal-enter-btn',
                    'button:contains("确定")',
                    ...
                    try {
                        ...
                        if (selector.includes(':contains')) {
                            ...
                            if (text) {
                                ...
                                const elements = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
                                const btn = elements.singleNodeValue;
            ...
            return findAndClickConfirm();
        }""")
        
        if result:
            print("  JavaScript点击已触发")
    ...
    if not confirm_clicked:
                        ...
                        if ((text.includes('确定') || text.includes('确认') || text.includes('Yes') || text.includes('OK')) && 
                            ...
                            setTimeout(() => {
                                try {
                                    ...
                                    resolve({ method: 'dialog', text: text });
                                } catch (e) {
                                    ...
                                    try {
                                        ...
                                        console.log('JavaScript事件派发成功');
                                        resolve({ method: 'dispatch', text: text });
                                    } catch (e2) {
                                        resolve(false);
                                    }
                                }
                    ...
                    if ((text.includes('确定') || text.includes('确认')) && 
                        ...
                        setTimeout(() => {
                            try {
                                ...
                                resolve({ method: 'global', text: text });
                            } catch (e) {
                                resolve(false);
                            }
                        }, 500);
        ...
        if result:
            print(f"  增强版JavaScript点击成功: {result}")
            await asyncio.sleep(1)  # 等待滚动和点击完成
            confirm_clicked = True
        ...
        try:
            ...
            dialog_exists = await page.evaluate("""() => {
                ...
                for (let d of dialogs) {
                    if (d.offsetParent !== null && d.textContent.includes('确定')) {
                        return true;
                    }
                }
                return false;
