...
async def fill_okr_section(page, part2_data):
    ...
    try:
        ...
        await page.evaluate("""() => {
            ...
            for (const h of headings) {
                if (h.textContent && h.textContent.includes('OKR进度')) {
                    ...
                    marker.style.position = 'fixed';
                    marker.style.left = '10px';
                    marker.style.top = h.getBoundingClientRect().top + 'px';
        ...
        # ==================== 第二步：使用类似第一部分的精确点击方法 ====================
        print("\n" + "-"*60)
        print("  [步骤2] 定位每个KR的输入区域")
        print("-"*60)
        
        # 我们先手动高亮所有可能的输入区域，让用户确认
        await page.evaluate("""() => {
            // 查找所有包含"本周工作"的区域
            ...
            elements.forEach(el => {
                if (el.textContent && el.textContent.includes('本周工作')) {
                    // 向上查找包含输入框的容器
                    let container = el;
                    for (let i = 0; i < 5; i++) {
                        ...
                        if (editable) {
                            ...
                            marker.style.zIndex = '10000';
                            marker.style.fontSize = '12px';
                            marker.textContent = `输入区域 ${inputCount}`;
                            document.body.appendChild(marker);
        ...
        print("\n  请查看页面上被绿色高亮的输入区域")
        print("  理想情况应该有 8 个输入区域（对应8个KR）")
        input("  确认高亮区域后按Enter继续...")
