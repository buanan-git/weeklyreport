    ...
    try:
        ...
        await page.evaluate("""() => {
            ...
            headings.forEach((h, index) => {
                if (h.textContent.includes('本周工作总结')) {
                    ...
                    label.style.padding = '2px 5px';
                    label.style.zIndex = '9999';
                    label.textContent = '本周工作总结标题';
                    h.appendChild(label);
                }
            });
        }""")
        
        print("\n  [交互] 已用红色边框高亮所有段落，蓝色边框高亮本周工作总结标题")
        input("  请确认页面上的段落位置，然后按Enter继续...")
        
        # 策略1：通过文本内容找到本周工作总结标题后的第一个空白段落
        print("\n  [尝试] 通过文本定位本周工作总结标题后的第一个空白段落")
        
        result = await page.evaluate("""(content) => {
            const editor = document.querySelector('.baidu__editor__content__right');
            if (!editor) {
                console.log('未找到主编辑区');
                return false;
            }
            ...
            for (const h of headings) {
                if (h.textContent.includes('本周工作总结')) {
                    targetHeading = h;
                    console.log('找到标题:', h.textContent);
                    break;
                }
            }
            
            if (!targetHeading) {
                console.log('未找到本周工作总结标题');
                return false;
            }
            ...
            // 收集接下来的几个段落
            while (nextElement && paragraphs.length < 5) {
                if (nextElement.classList && nextElement.classList.contains('mp-paragraph-wrapper')) {
                    paragraphs.push(nextElement);
                }
                nextElement = nextElement.nextElementSibling;
            }
            
            console.log('找到标题后的段落数量:', paragraphs.length);
            ...
            if (paragraphs.length > 0) {
                ...
                // 可视化选中
                targetPara.style.backgroundColor = '#90EE90';
                targetPara.style.border = '3px solid green';
                ...
                targetPara.appendChild(spanWrapper);
                
                targetPara.dispatchEvent(new Event('input', {bubbles: true}));
                targetPara.dispatchEvent(new Event('change', {bubbles: true}));
        ...
        if result:
            print("  [OK] 已定位并填写内容，目标段落已高亮为绿色")
            input("  请确认内容是否填写在正确位置（本周工作总结下的第一个空白段落），然后按Enter继续...")
            
            # 清除高亮
            await page.evaluate("""() => {
                document.querySelectorAll('.mp-paragraph-wrapper').forEach(p => {
                    p.style.border = '';
                    p.style.backgroundColor = '';
                    // 移除我们添加的序号标签
                    const labels = p.querySelectorAll('div');
                    labels.forEach(l => l.remove());
                });
                document.querySelectorAll('h3.mp-heading, h1.mp-heading').forEach(h => {
                    h.style.border = '';
                    h.style.backgroundColor = '';
                    const labels = h.querySelectorAll('div');
                    labels.forEach(l => l.remove());
                });
            }""")
