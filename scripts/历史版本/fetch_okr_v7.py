...
def parse_weekly_report_structure_v7(full_text):
    """
    V7核心：解析三部分结构
    ...
    if okr_header_index >= 0:
        ...
        # 增强的正则：匹配 O1/O2/O3 或 O1：或 O1: 或 O1 开头，允许前面有空格
        o_pattern = re.compile(r'^\s*[Oo](\d+)\s*[:：\s]?(.*)')
        # 增强的正则：匹配 KR1/KR2/KR3 或 KR1：或 KR1: 或 KR1 开头，允许前面有空格
        ...
        while i < len(okr_lines):
            ...
            if o_match:
                ...
                # 提取整行内容作为标题（包含O1前缀和后续所有内容）
                o_title = line.strip()
                print(f"    [调试] 检测到O{o_num}标题: {o_title[:80]}")
            ...
            # 检测KR（三级标题）- 提取整行内容作为标题
            kr_match = kr_pattern.match(line)
            if kr_match and current_o:
                kr_num = kr_match.group(1)
                # 提取整行内容作为标题（包含KR1前缀和后续所有内容）
                kr_title = line.strip()
                print(f"    [调试] 检测到KR{kr_num}标题: {kr_title[:80]}")
            ...
            # 检测"本周工作"开头的输入框内容
            if re.match(r'^(?:本周工作|本周进展|工作进展)[:：\s]*(.*)', line, re.IGNORECASE):
                # 提取"本周工作"后的内容
                work_match = re.match(r'^(?:本周工作|本周进展|工作进展)[:：\s]*(.*)', line, re.IGNORECASE)
                if work_match and work_match.group(1).strip():
                    if current_kr:
                        current_kr['weekly_work'].append(work_match.group(1).strip())
                # 继续读取后续行，直到遇到下一个O/KR/标题
                i += 1
                while i < len(okr_lines):
                    ...
                    if o_pattern.match(next_line) or kr_pattern.match(next_line) or \
                       any(keyword in next_line for keyword in ['业务核心指标', '主要项目', '下周重点工作']):
                        break
                    # 如果内容足够长，添加到当前KR的本周工作
                    if len(next_line) > 5:
                        if current_kr:
                            current_kr['weekly_work'].append(next_line)
                    i += 1
                continue
            
            # 如果当前在KR中，且不是新的O或KR，收集为本周工作内容
            if current_kr and not o_pattern.match(line) and not kr_pattern.match(line):
                if len(line) > 5 and '本周工作' not in line:
...
async def scroll_container_v7(page, selector, center):
    """V7：确保内容完整加载（保持原有代码）"""
...
async def fetch_single_staff_v7(page, staff_id, date, desc, index=0, total=0):
    """
    V7单个人员抓取（抽取为独立函数便于重试）
    新增：首次页面加载后等待5秒
    修复：更完善的登录状态检查
    """
    ...
    try:
        ...
        await scroll_container_v7(page, selector, center)
        print("✓")
        ...
        print("  解析三部分结构...", end=" ")
        parsed = parse_weekly_report_structure_v7(full_text)
...
async def fetch_staff_v7_with_retry(page, staff_id, date, desc, index=0, total=0):
    """
    V7抓取（带重试机制）
    失败后记录，稍后重试（最多1次）
    """
    result = await fetch_single_staff_v7(page, staff_id, date, desc, index, total)
    return result


# ==================== 整合功能（V7格式）====================

def integrate_weekly_reports_v7():
    """V7整合 - 基于三部分结构"""
    print("\n" + "="*60)
    print("整合周报...")
    ...
    # 生成报告
    team_md = generate_team_weekly_v7(reports)
    with open('team_weekly_report_v7.md', 'w', encoding='utf-8') as f:
        f.write(team_md)
    print(f"  ✓ 团队周报: team_weekly_report_v7.md")
    
    llm_prompt = generate_llm_prompt_v7(reports)
    with open('llm_prompt_v7.txt', 'w', encoding='utf-8') as f:
        f.write(llm_prompt)
    print(f"  ✓ 提示词: llm_prompt_v7.txt")
    
    personal_md = generate_personal_reference_v7(reports)
    with open('personal_reference_v7.md', 'w', encoding='utf-8') as f:
        f.write(personal_md)
    print(f"  ✓ 个人参考: personal_reference_v7.md")
    
    return {'team_md': team_md, 'llm_prompt': llm_prompt, 'reports': reports}

def generate_team_weekly_v7(reports):
    """V7生成团队周报 - 按照三部分结构"""
    lines = [
        "# 团队周报汇总 - V7三部分结构",
        "",
...
def generate_llm_prompt_v7(reports):
    """V7生成提示词（三部分结构）"""
    lines = [
...
def generate_personal_reference_v7(reports):
    """V7生成个人参考（三部分结构）"""
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for i, staff_id in enumerate(STAFF_IDS, 1):
            result = await fetch_single_staff_v7(page, staff_id, last_week, f"下属{i}", i, len(STAFF_IDS))
        ...
        if not any(r['status'] == 'login_required' for r in all_results):
            ...
            r1 = await fetch_single_staff_v7(page, MY_ID, last_week, "本人-上周", 1, 2)
            ...
            if r1['status'] != 'login_required':
                await asyncio.sleep(2)
                r2 = await fetch_single_staff_v7(page, MY_ID, week_before, "本人-上上周", 2, 2)
        ...
        if failed_tasks:
            ...
            for task in failed_tasks:
                print(f"\n重试: {task['desc']}")
                result = await fetch_single_staff_v7(
                    page, 
                    task['staff_id'], 
                    task['date'], 
                    task['desc'] + "(重试)", 
        ...
        with open('v7_fetch_summary.json', 'w', encoding='utf-8') as f:
        ...
        # 自动整合
        integrate_result = integrate_weekly_reports_v7()
        ...
        if integrate_result:
            ...
            print(f"  - team_weekly_report_v7.md (V7三部分结构团队周报)")
            print(f"  - llm_prompt_v7.txt (大模型提示词)")
            print(f"  - personal_reference_v7.md (个人历史参考)")
            print(f"  - v7_fetch_summary.json (抓取汇总)")
