...
async def fill_by_javascript(page, label_text, content):
    ...
    print(f"    [DEBUG-JS] 内容前100字符: {repr(content[:100])}")

    try:
        # 使用page.evaluate的独立参数传递，避免字符串拼接问题
        result = await page.evaluate("""(args) => {
            ...
            if (input.tagName === 'TEXTAREA' || input.tagName === 'INPUT') {
                input.value = content;
                console.log('JS: 已设置value, 长度:', content.length);
            } else {
                // 处理contenteditable元素
                // 将换行符转换为<br>标签
            ...
            return 'ok';
        }""", {"labelText": label_text, "content": content})
