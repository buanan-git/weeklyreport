...
def get_config_value(*keys, default=None):
    """
    通用配置获取函数，优先从 llm_chat 获取，回退到原有配置
    """
    # 优先从 llm_chat 获取
    llm_chat = config.config.get("llm_chat", {})
    for key in keys:
        if key in llm_chat:
            return llm_chat[key]
    
    # 回退到原有配置路径
    if len(keys) == 1 and keys[0] == "timeout":
        # 尝试从平台配置获取
        pass
    elif len(keys) == 1 and keys[0] == "temperature":
        pass
    
    return default

def get_prompt_file_path() -> Optional[Path]:
    """
    获取提示词文件路径
    优先级: 
    1. llm_chat.prompt_file + llm_chat.prompts_dir
    2. config.optimize_prompt (原有配置)
    3. 默认值
    """
    llm_chat = config.config.get("llm_chat", {})
    ...
    if llm_chat.get("prompt_file"):
        prompts_dir = llm_chat.get("prompts_dir", "config/prompts")
        # 构建完整路径
        if Path(prompts_dir).is_absolute():
            base_dir = Path(prompts_dir)
        else:
            base_dir = Path(__file__).parent.parent / prompts_dir
        
        prompt_path = base_dir / llm_chat["prompt_file"]
        if prompt_path.exists():
            return prompt_path
    
    # 2. 回退到原有配置
    try:
        return config.optimize_prompt
    except:
        pass
    
    # 3. 默认值
    default_path = Path(__file__).parent.parent / "config" / "prompts" / "WeeklyReportOptimizePrompt.md"
    return default_path

# ==================== 原有的 UnifiedAPIClient 类保持不变 ====================
...
class UnifiedAPIClient:
    ...
    def __init__(self, platform: str):
        ...
        # API调用配置 - 优先使用 llm_chat 的全局配置，再使用平台特定配置
        self.timeout = (
            get_config_value("timeout") or 
            self.api_config.get("timeout", 300)
        )
        self.max_tokens = (
            get_config_value("max_tokens") or 
            self.api_config.get("max_tokens", 4000)
        )
        self.temperature = (
            get_config_value("temperature") or 
            self.api_config.get("temperature", 0.7)
        )
...
async def llm_api_resp(
    ...
    # 处理超时参数 - 优先使用传入的，然后从配置获取
    if timeout is None:
        timeout = get_config_value("timeout", default=300)
    ...
    try:
        ...
        log_msg(f"数据大小: {len(json.dumps(attachment_data, ensure_ascii=False))} 字符")
    except Exception as e:
    ...
    if prompt_file and os.path.exists(prompt_file):
        ...
        log_msg(f"使用传入的提示词文件: {prompt_path}")
    else:
        # 从配置获取提示词文件
        prompt_path = get_prompt_file_path()
        if prompt_path and prompt_path.exists():
            log_msg(f"从配置获取提示词文件: {prompt_path}")
        else:
            prompt_path = None
    ...
    if prompt_path:
        try:
            ...
            log_msg(f"提示词加载成功，长度: {len(prompt_content)} 字符")
        except Exception as e:
...
async def main():
    ...
    parser.add_argument("-t", "--timeout", type=int, default=None, help="超时时间")
