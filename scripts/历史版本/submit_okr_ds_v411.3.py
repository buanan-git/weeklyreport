...
async def fill_okr_section(page, part2_data):
    ...
    try:
        ...
        table_info = await page.evaluate("""() => {
            ...
            const processedInputs = new Set(); // 去重
        ...
        for i, inp in enumerate(debug_inputs):
            ...
            print(f"      ID: {inp['id']}")
            print(f"      父元素标签: {inp['parentTag']}")
            print(f"      父元素类: {inp['parentClass'][:100]}")
            print(f"      表格容器类: {inp['containerClass'][:100]}")
            print(f"      坐标: ({inp['coordinates']['x']:.0f}, {inp['coordinates']['y']:.0f})")
            print(f"      原始Y: {inp['coordinates']['rawY']:.0f}")
        # ==== 结束新增打印 ====
        ...
        await asyncio.sleep(1)
        input("\n  请确认：1.蓝色OKR进度标题 2.红色表格区域 3.绿色输入区域（共10个），按Enter继续...")
