...
async def fill_okr_section(page, part2_data):
    """填写第二部分：OKR进度 - 精确定位激活版"""
    
    print("\n" + "="*80)
    print("[填写] 第二部分：OKR进度（精确定位激活版）")
    print("="*80)
    ...
    try:
        ...
        for order_idx, kr_info in enumerate(fill_order):
            ...
            # 交互确认
            response = input(f"    是否定位到 O{kr_info['o']}KR{kr_info['kr']} 的输入框？(y=确认并填写, n=跳过, q=终止): ").strip().lower()
            ...
            if not activated:
                print("      [❌] 无法自动激活，请手动点击输入框")
                input("        请手动点击目标输入框，然后按Enter继续...")
...
async def fill_part3_metrics(page, part3_data):
    ...
    total_sections = len([s for s in section_names if s in sections])
    ...
    try:
        ...
        section_info = await page.evaluate("""() => {
            ...
            result.success = true;
            result.message = `找到 ${Object.keys(result.sections).length} 个区域`;
            return result;
        }""")
        ...
        for section_name in section_names:
            ...
            if section_name not in section_info['sections']:
                print(f"\n  [跳过] {section_name}: 未找到页面位置")
                continue
            ...
            # 交互确认
            response = input(f"    是否定位到 {section_name}？(y=确认并填写, n=跳过, q=终止): ").strip().lower()
...
async def auto_submit_and_confirm(page):
    ...
    if not submit_clicked:
        ...
        result = await page.evaluate("""() => {
            ...
            for (let btn of buttons) {
                ...
                if ((text.includes('保存') || text.includes('提交') || text.includes('更新')) && 
                    btn.offsetParent !== null && 
                    window.getComputedStyle(btn).display !== 'none') {
                    btn.click();
                    console.log('JavaScript点击:', text);
                    return text;
                }
            }
            return false;
    ...
    print("\n[确认] 处理确认弹框...")
    await asyncio.sleep(2)  # 等待弹框出现

    confirm_clicked = False
    
    # 方法1: 通过 role="dialog" 查找确认按钮
    try:
        # 先查找对话框
        ...
        if await dialog.count() > 0:
            ...
            # 在对话框中查找确认按钮
            for text in ["确认", "确定", "Yes", "OK", "Confirm", "好的", "是"]:
                try:
                    confirm_btn = dialog.get_by_role("button", name=text).first
                    if await confirm_btn.count() > 0 and await confirm_btn.is_visible(timeout=1000):
                        print(f"  点击确认: {text}")
    ...
    # 方法3: JavaScript兜底查找确认按钮
    if not confirm_clicked:
        print("  尝试JavaScript查找确认按钮...")
        result = await page.evaluate("""() => {
            // 查找所有可能包含确认按钮的容器
            const dialogs = document.querySelectorAll('[role="dialog"], .modal, .ant-modal, .el-dialog, .confirm-dialog, [class*="dialog"], [class*="modal"]');
            
            for (let dialog of dialogs) {
                if (dialog.style.display === 'none' || window.getComputedStyle(dialog).display === 'none') continue;
                
                const buttons = dialog.querySelectorAll('button, [role="button"], .btn, .ant-btn, .el-button');
                for (let btn of buttons) {
                    const text = (btn.textContent || btn.value || btn.getAttribute('aria-label') || '').trim();
                    if ((text.includes('确认') || text.includes('确定') || text.includes('Yes') || text.includes('OK')) && 
                        btn.offsetParent !== null && 
                        window.getComputedStyle(btn).display !== 'none') {
                        btn.click();
        ...
        if result:
            confirm_clicked = True
            print(f"  JavaScript点击确认: {result}")

    # 等待确认完成
    if confirm_clicked:
        print("  [OK] 确认完成")
        await asyncio.sleep(2)  # 等待弹框消失
        
        # 可选：检查是否还有弹框
        try:
            dialog = page.get_by_role("dialog").first
            if await dialog.count() > 0 and await dialog.is_visible(timeout=2000):
                print("  [⚠️] 对话框仍然存在，可能需要额外处理")
        except:
            pass
        ...
        print("\n  [调试] 当前可见按钮:")
        for btn in buttons_info[:10]:  # 只显示前10个
            if btn['text']:
                print(f"    - {btn['text']}")
