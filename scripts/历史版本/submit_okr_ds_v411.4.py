...
async def fill_okr_section(page, part2_data):
    """填写第二部分：OKR进度 - 参考第一部分激活逻辑"""
    
    print("\n" + "="*80)
    print("[填写] 第二部分：OKR进度（参考第一部分激活逻辑）")
    print("="*80)
    ...
    try:
        ...
        input("\n  请确认绿色输入框共8个，按Enter开始定位确认...")
        
        # ==================== 第二步：逐个确认并填写（参考第一部分激活逻辑） ====================
        print("\n" + "-"*60)
        print("  [步骤2] 逐个确认并填写")
        print("-"*60)
        ...
        for order_idx, kr_info in enumerate(fill_order):
            ...
            print(f"    目标输入框 {order_idx + 1}: 坐标({input_info['x']:.0f}, {input_info['y']:.0f})")
            ...
            click_x = input_info['x']
            click_y = input_info['y']
            ...
            for strategy_name, click_count in click_strategies:
                ...
                if is_active:
                    print(f"      [OK] {strategy_name}成功激活编辑器")
                    activated = True
                    break
            
            if not activated:
                print("      [⚠️] 自动点击未能激活，请手动点击")
            ...
            if weekly_work[:20] in after_content:
                print(f"    [✅] O{kr_info['o']}KR{kr_info['kr']} 填写完成")
                success_count += 1
            else:
                print(f"    [❌] 输入可能失败，请检查")
                response = input("      输入是否成功？(y=确认成功, n=标记失败): ").strip().lower()
                if response == 'y':
                    success_count += 1
