...
async def fill_part1_work_summary(page, part1_data):
    ...
    if await anchor.count() > 0:
        ...
        if box:
            ...
            # X 对齐“本”字的左侧起始位
            target_x = box['x'] + 5 
            # Y 移动到文字下方约 15-20 像素 (根据你反馈的起始位置)
            target_y = box['y'] + box['height'] + 15 

            # 高亮红点
            await draw_debug_dot(page, target_x, target_y)

            iprint(f"红点已对齐'本'字下方 15px。坐标: ({int(target_x)}, {int(target_y)})")
            await ask_user("请确认红点是否在输入框起始位置？确认后将尝试激活并输入。")

            # --- 物理激活逻辑 ---
            # 先移动到位置
            await page.mouse.move(target_x, target_y)
            # 点击并稍微抖动一下鼠标，确保触发富文本编辑器的 focus 监听
            await page.mouse.down()
            await page.mouse.move(target_x + 2, target_y) 
            await page.mouse.up()
            ...
            # 2. 焦点隔离 (防止侧栏劫持)
            await page.evaluate("""() => {
                document.querySelectorAll('aside, .sidebar, [class*="drawer"]').forEach(el => {
            ...
            # 恢复设置
            await page.evaluate("""() => {
                const dot = document.getElementById('ai-target-dot');
                if(dot) dot.style.display = 'none';
                document.querySelectorAll('aside, .sidebar').forEach(el => el.style.pointerEvents = 'auto');
            }""")
            
            await ask_user("内容已追加。请确认：1. 是否有闪动光标？2. 原有内容是否保留？")
            return True

    return False

async def fill_part2_okr_weekly_work(page, part2_data):
    """第二部分：OKR 进度填报（高频交互确认版）"""
    iprint("\n[填写] 第二部分：OKR 进度更新")
    ...
    for o_idx, obj in enumerate(objectives):
        o_title = obj.get("o_title", "未命名目标")
        for kr_idx, kr in enumerate(obj.get("key_results", [])):
            kr_id = f"KR{kr_idx + 1}"
            weekly_content = kr.get("weekly_work", "")
            if not weekly_content: continue
            ...
            if await kr_anchor.count() > 0:
                ...
                await ask_user(f"已发现锚点 [{kr_id}] (已高亮)，是否继续在该 KR 下寻找输入框？")
            else:
                iprint(f"  [X] 未找到锚点 {kr_id}，跳过该项")
                continue

            # --- 步骤 2: 寻找“本周工作”标签 ---
            # 这里的定位逻辑是：找 KR 锚点之后出现的第一个“本周工作”
            ...
            if await work_label.count() > 0:
                ...
                # 高亮标签文字
                await work_label.evaluate("el => el.style.border = '2px dashed blue'")
                await ask_user(f"已锁定 [{kr_id}] 下方的‘本周工作’标签，是否计算输入框坐标？")

                # --- 步骤 3: 物理坐标定位确认 ---
                # X: 对齐“本”字左侧；Y: 下方约 15 像素
                target_x = l_box['x'] + 5 
                target_y = l_box['y'] + l_box['height'] + 15

                # 绘制红点
                await draw_debug_dot(page, target_x, target_y)
                await page.mouse.move(target_x, target_y)
                
                await ask_user(f"红点已指向 [{kr_id}] 输入框起始位。确认点击激活并输入内容？")
                ...
                # 隔离侧栏焦点
                await page.evaluate("() => document.querySelectorAll('aside, .sidebar').forEach(el => el.style.pointerEvents = 'none')")
                ...
                # 输入内容
                iprint(f"  [注入] 正在输入 {kr_id} 的周报内容...")
                await page.keyboard.insert_text(weekly_content)

                # 恢复侧栏
                await page.evaluate("() => document.querySelectorAll('aside, .sidebar').forEach(el => el.style.pointerEvents = 'auto')")
                
                # 清除当前高亮以便下一步
                await work_label.evaluate("el => { el.style.border = ''; }")
                await page.evaluate("document.getElementById('ai-target-dot').style.display = 'none'")

                # --- 步骤 5: 结果确认 ---
                await ask_user(f"{kr_id} 填写尝试完成。请检查是否保留了原有内容？确认后将处理下一项。")
            else:
                iprint(f"  [!] 在 {kr_id} 附近未发现‘本周工作’字样，请手动检查页面布局。")
