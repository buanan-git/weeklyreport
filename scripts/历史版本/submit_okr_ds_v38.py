...
async def fill_part1_work_summary(page, part1_data):
    """填写第一部分：本周工作总结 - 调整光标距离版"""
    
    print("\n" + "="*80)
    print("[填写] 第一部分：本周工作总结（调整距离版）")
    print("="*80)
    ...
    try:
        ...
        await page.evaluate("""() => {
            document.querySelectorAll('[style*="border"]').forEach(el => {
                if (el.style.border.includes('2px solid red')) {
                    el.style.border = '';
        ...
        title_info = await page.evaluate("""() => {
            ...
            if (!editor) {
                result.message = '未找到编辑器';
                return result;
            }
            
            // 找到H1标题
            const headings = editor.querySelectorAll('h1, h2, h3, .mp-heading');
            ...
            titleElement.style.backgroundColor = 'rgba(255, 255, 0, 0.2)';
            ...
            result.fontSize = parseInt(style.fontSize) || 16;
            
            // 计算一个字符的宽度（中文字符大约等于字体大小）
            const charWidth = result.fontSize;
            
            // 点击位置：标题右侧内部 + 2个字符的距离
            // 避免太近选中文字，也不要太远
            result.clickX = rect.right - 20 + (charWidth * 2);
            ...
            marker.style.border = '2px solid #00AA00';
            ...
            label.style.left = (result.clickX + 15) + 'px';
            label.style.top = (result.clickY - 20) + 'px';
            ...
            label.textContent = '点击这里（距离文字2字符）';
            document.body.appendChild(label);
            
            result.success = true;
            result.message = '已找到标题位置';
            return result;
        }""")
        ...
        print(f"  点击坐标: ({title_info['clickX']}, {titleInfo['clickY']})")
        print(f"  字体大小: {title_info['fontSize']}px")
        
        input("\n  请确认绿色圆点位置（应在'结'字后方2字符距离），按Enter继续...")
    ...
    try:
        ...
        title_info = await page.evaluate("""() => {
            ...
            // 计算点击位置：标题右侧内部（避开可能的折叠按钮）
            // 假设标题文字大约占宽度的80%
            const textWidth = rect.width * 0.8;
            result.clickX = rect.left + textWidth + 5;  // 文字末尾再加5像素
            result.clickY = rect.top + rect.height / 2;
            ...
            marker.style.zIndex = '1000000';
            marker.style.pointerEvents = 'none';
            marker.style.boxShadow = '0 0 10px green';
            document.body.appendChild(marker);
        ...
        print(f"  [OK] 找到标题: {title_info['titleText']}")
        print(f"  点击位置: ({title_info['clickX']}, {title_info['clickY']})")
        
        input("\n  请确认绿色圆点标记的位置是否在'结'字后方，按Enter继续...")
        ...
        has_test = await page.evaluate("""() => {
            const editor = document.querySelector('[data-slate-editor="true"]');
            if (!editor) return false;
            return editor.textContent.includes('test');
        }""")
