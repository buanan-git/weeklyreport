...
百度OKR周报抓取 - V8表格结构修复版
基于V7修复以下问题：
1. O1/O2/O3标题提取不完整 - 现在正确提取整行内容
2. KR标题被错误放入weekly_work - 现在正确放入title字段
3. 本人和下属抓取逻辑完全一致
...
def parse_weekly_report_structure_v8(full_text):
    """
    V8核心：解析三部分结构
    
    V8修复：
    1. O1/O2/O3标题正确提取整行内容
    2. KR1/KR2/KR3标题正确放入title字段，不混入weekly_work
    ...
    第二部分：OKR进度（三级表格结构）
    - 第一行：总标题（如"张三的OKR"）
    - 第二行：O1标题（Object）- V8:提取整行
    - 第三行：KR1标题（Key Result）- V8:提取整行到title
    - 第四行：本周工作输入框内容
    - 后续：O1的其他KR，然后是O2、O3...
    ...
    if okr_header_index >= 0:
        ...
        # V8: 简化的正则，只匹配O1/O2/O3或KR1/KR2/KR3开头
        ...
        while i < len(okr_lines):
            ...
            if any(keyword in line for keyword in ['业务核心指标', '主要项目', '下周重点工作']):
                print(f"    [调试] 检测到第三部分标题，停止OKR解析")
                break
            
            # 检测O（二级标题）- V8: 提取整行作为标题
            o_match = o_pattern.match(line)
            if o_match:
                o_num = o_match.group(1)
                o_title = original_line.strip()  # V8: 使用整行原始内容
                print(f"    [调试] O{o_num}标题: {o_title[:100]}")
            ...
            # 检测KR（三级标题）- V8: 提取整行作为标题
            kr_match = kr_pattern.match(line)
            if kr_match and current_o:
                kr_num = kr_match.group(1)
                kr_title = original_line.strip()  # V8: 使用整行原始内容
                print(f"    [调试] KR{kr_num}标题: {kr_title[:100]}")
                
                current_kr = {
                    'level': 2,
                    'id': f'KR{kr_num}',
                    'title': kr_title,
                    'weekly_work': []  # V8: 确保weekly_work初始为空
                }
            ...
            if weekly_work_match and current_kr:
                ...
                if content:
                    print(f"    [调试] 本周工作: {content[:60]}")
                    current_kr['weekly_work'].append(content)
                
                # 继续读取后续行，直到遇到下一个O/KR/第三部分标题
                i += 1
                while i < len(okr_lines):
                    next_line = okr_lines[i]
                    # 如果遇到新的O/KR或第三部分标题，停止
                    if o_pattern.match(next_line) or kr_pattern.match(next_line) or \
                       any(keyword in next_line for keyword in ['业务核心指标', '主要项目', '下周重点工作']):
                        break
                    # 添加到当前KR的本周工作
                    if len(next_line) > 3:  # V8: 降低长度阈值
                        print(f"    [调试] 本周工作(续): {next_line[:60]}")
                        current_kr['weekly_work'].append(next_line.strip())
                    i += 1
                continue
            
            # V8: 只有在确认不是O/KR/本周工作标题后，才收集为工作内容
            if current_kr and len(line) > 5:
                # 再次检查确保不是O/KR标题（防止误判）
                if not o_pattern.match(line) and not kr_pattern.match(line) and \
                   not re.match(r'^(?:本周工作|本周进展|工作进展)', line, re.IGNORECASE):
                    print(f"    [调试] KR工作内容: {line[:60]}")
...
async def scroll_container_v8(page, selector, center):
    """V8：确保内容完整加载"""
...
async def fetch_single_staff_v8(page, staff_id, date, desc, index=0, total=0):
    """
    V8单个人员抓取（抽取为独立函数便于重试）
    新增：首次页面加载后等待5秒
    修复：更完善的登录状态检查
    V8修复：O/KR标题正确提取，本人和下属逻辑一致
    """
    ...
    try:
        ...
        await scroll_container_v8(page, selector, center)
        print("✓")
        ...
        print("  解析三部分结构...", end=" ")
        parsed = parse_weekly_report_structure_v8(full_text)
...
async def fetch_staff_v8_with_retry(page, staff_id, date, desc, index=0, total=0):
    """
    V8抓取（带重试机制）
    失败后记录，稍后重试（最多1次）
    """
    result = await fetch_single_staff_v8(page, staff_id, date, desc, index, total)
    return result


# ==================== 整合功能（V8格式）====================

def integrate_weekly_reports_v8():
    """V8整合 - 基于三部分结构"""
    print("\n" + "="*60)
    print("整合周报...")
    ...
    # 生成报告
    team_md = generate_team_weekly_v8(reports)
    with open('team_weekly_report_v8.md', 'w', encoding='utf-8') as f:
        f.write(team_md)
    print(f"  ✓ 团队周报: team_weekly_report_v8.md")
    
    llm_prompt = generate_llm_prompt_v8(reports)
    with open('llm_prompt_v8.txt', 'w', encoding='utf-8') as f:
        f.write(llm_prompt)
    print(f"  ✓ 提示词: llm_prompt_v8.txt")
    
    personal_md = generate_personal_reference_v8(reports)
    with open('personal_reference_v8.md', 'w', encoding='utf-8') as f:
        f.write(personal_md)
    print(f"  ✓ 个人参考: personal_reference_v8.md")
    
    return {'team_md': team_md, 'llm_prompt': llm_prompt, 'reports': reports}

def generate_team_weekly_v8(reports):
    """V8生成团队周报 - 按照三部分结构"""
    lines = [
        "# 团队周报汇总 - V8三部分结构（O/KR标题修复版）",
        "",
...
def generate_llm_prompt_v8(reports):
    """V8生成提示词（三部分结构）"""
    lines = [
...
def generate_personal_reference_v8(reports):
    """V8生成个人参考（三部分结构）"""
    my_reports = [r for r in reports if '本人' in r.get('desc', '')]
    
    lines = [
        "# 个人历史周报参考 - V8三部分结构（O/KR标题修复版）",
        "",
        f"**生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
        "",
        "---",
...
async def main():
    ...
    print("="*60)
    print(" "*15 + "百度OKR周报抓取 - V8表格结构修复版")
    print("="*60)
    print("\n【V8修复】")
    print("  ✓ O1/O2/O3标题正确提取整行内容")
    print("  ✓ KR1/KR2/KR3标题正确放入title字段（不混入weekly_work）")
    print("  ✓ 本人和下属抓取逻辑完全一致")
    ...
    async with async_playwright() as p:
        ...
        for i, staff_id in enumerate(STAFF_IDS, 1):
            result = await fetch_single_staff_v8(page, staff_id, last_week, f"下属{i}", i, len(STAFF_IDS))
        ...
        if not any(r['status'] == 'login_required' for r in all_results):
            ...
            r1 = await fetch_single_staff_v8(page, MY_ID, last_week, "本人-上周", 1, 2)
            ...
            if r1['status'] != 'login_required':
                await asyncio.sleep(2)
                r2 = await fetch_single_staff_v8(page, MY_ID, week_before, "本人-上上周", 2, 2)
        ...
        if failed_tasks:
            ...
            for task in failed_tasks:
                print(f"\n重试: {task['desc']}")
                result = await fetch_single_staff_v8(
                    page, 
                    task['staff_id'], 
                    task['date'], 
                    task['desc'] + "(重试)", 
        ...
        with open('v8_fetch_summary.json', 'w', encoding='utf-8') as f:
        ...
        # 自动整合
        integrate_result = integrate_weekly_reports_v8()
        ...
        if integrate_result:
            ...
            print(f"  - team_weekly_report_v8.md (V8三部分结构团队周报 - O/KR标题修复)")
            print(f"  - llm_prompt_v8.txt (大模型提示词)")
            print(f"  - personal_reference_v8.md (个人历史参考)")
            print(f"  - v8_fetch_summary.json (抓取汇总)")
