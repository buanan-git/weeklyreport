...
class OKRSubmitter:
    ...
    async def fill_part1_work_summary(self, content: str):
        ...
        try:
            if main_area:
                ...
                if await work_summary_heading.count() > 0:
                    ...
                    # 找到该标题后的第一个编辑器
                    # 策略A: 直接兄弟元素
                    editor = work_summary_heading.locator('xpath=following-sibling::div[contains(@class,"editor") or @contenteditable="true"][1]').first
                    ...
                    if await editor.count() == 0:
                        # 策略B: 父容器内的编辑器
                        parent = work_summary_heading.locator('xpath=ancestor::section').first
                        if await parent.count() == 0:
                            parent = work_summary_heading.locator('xpath=ancestor::div[contains(@class,"section")]').first
                        if await parent.count() == 0:
                            parent = work_summary_heading.locator('xpath=ancestor::div[contains(@class,"card")]').first
                        if await parent.count() > 0:
        ...
        try:
            # 策略3: 通过文本标签的层级结构 - 修复CSS选择器语法
            # CSS选择器不能使用 |，需要使用逗号分隔
            section1 = self.page.locator('div:has-text("重要会议总结"):not(:has(div:has-text("辅助区")))').first
            section2 = self.page.locator('div:has-text("工作总结"):not(:has(div:has-text("辅助区")))').first
    ...
    async def fill_okr_section(self, okr_data: Any):
        """填写第二部分：OKR进度 - 基于KR标签定位"""
        print("\n📝 正在填写【OKR进度】...")

        # 处理数据类型 - 可能是字典或字符串
        if isinstance(okr_data, str):
            print(f"   ⚠️ OKR数据是字符串格式，尝试解析")
            try:
                okr_data = json.loads(okr_data)
            except:
                print(f"   ⚠️ 无法解析为JSON，作为纯文本处理")
                # 尝试找到第一个KR输入框填入
                main_area = await self.find_main_content_area()
                if main_area:
                    okr_section = main_area.locator('div:has-text("OKR进度"), section:has-text("OKR进度")').first
                    if await okr_section.count() > 0:
                        first_editor = okr_section.locator('div[contenteditable="true"]').first
                        if await first_editor.count() > 0:
                            await self.smart_fill(first_editor, okr_data)
                            return True
                return False

        if not okr_data or not isinstance(okr_data, dict):
            print("   ⚠️ OKR数据为空或格式不正确，跳过")
            return True
        ...
        for kr_key, kr_content in okr_data.items():
            ...
            try:
                ...
                kr_locator = None
                kr_filter = has_text=re.compile(rf"^{kr_key}\b|\b{kr_key}\b")

                if main_area:
                    kr_locator = main_area.locator(f'div:has-text("{kr_key}"):not(:has(div:has-text("辅助区")))').filter(kr_filter).first
                else:
                    kr_locator = self.page.locator(f'div:has-text("{kr_key}"):not(:has(div:has-text("辅助区")))').filter(kr_filter).first

                if await kr_locator.count() > 0:
                    print(f"      ✅ 找到{kr_key}标签")

                    # 向上找到KR容器（行或卡片）
                    kr_container = kr_locator.locator('xpath=ancestor::div[contains(@class,"kr") or contains(@class,"row") or contains(@class,"item") or contains(@class,"card")][1]').first
                    if await kr_container.count() == 0:
                        kr_container = kr_locator.locator('xpath=ancestor::tr[1]').first
                    ...
                    if await kr_container.count() > 0:
                        ...
                        if await work_label.count() > 0:
                            # 找到对应的输入框
                            editor = work_label.locator('xpath=following::div[contains(@class,"editor") or @contenteditable="true"][1]').first
                            if await editor.count() == 0:
                                editor = work_label.locator('xpath=following-sibling::div[contains(@class,"editor") or @contenteditable="true"][1]').first
                            if await editor.count() == 0:
                                editor = work_label.locator('xpath=ancestor::div[1]//div[contains(@class,"editor") or @contenteditable="true"]').first
            ...
            try:
                ...
                if main_area:
                    ...
                    if await okr_section.count() > 0:
                        ...
                        kr_match = re.search(r"KR(\d+)", kr_key)
                        kr_index = int(kr_match.group(1)) - 1 if kr_match else 0
            ...
            try:
                ...
                if main_area:
                    ...
                    kr_match = re.search(r"KR(\d+)", kr_key)
                    kr_index = int(kr_match.group(1)) if kr_match else 1
                    target_index = kr_index
