...
async def fill_part1_work_summary(page, part1_data):
    ...
    if await anchor.count() > 0:
        ...
        if box:
            ...
            iprint(f"  [定位] 已对齐总结起始位: ({int(target_x)}, {int(target_y)})")
            await ask_user("红点已对齐起始位。确认后将执行‘先顶开下方内容、再从顶端输入’逻辑。")

            # 2. 物理点击激活并锁定光标
            await page.mouse.click(target_x, target_y)
            await asyncio.sleep(0.8) # 等待页面脚本反应

            # 3. 顶开下方已有内容：先按两次回车
            iprint("  [动作] 正在向下挤压原有内容...")
            await page.keyboard.press("Enter")
            await page.keyboard.press("Enter")
            await asyncio.sleep(0.3)

            # 4. 强制回到顶端起始位进行输入
            # 连续按向上键确保光标回到最顶部，不跳出输入框
            await page.keyboard.press("ArrowUp")
            await page.keyboard.press("ArrowUp")
            await page.keyboard.press("Home")

            # 5. 执行物理文字注入
            iprint("  [注入] 正在起始位置输入周报内容...")
            await page.keyboard.insert_text(content)
            
            await ask_user("第一部分填写完毕。请确认新增内容是否已置顶且未干扰下方原有内容？")
...
async def fill_part2_okr_weekly_work(page, part2_data):
    ...
    for obj in objectives:
        for kr_idx, kr in enumerate(obj.get("key_results", [])):
            ...
            if not kr.get("weekly_work"): continue
            ...
            if await kr_anchor.count() > 0:
                ...
                if await work_label.count() > 0:
                    ...
                    await ask_user(f"红点已对齐 [{kr_id}] ‘本’字下方 5px。准备执行十字探测并挤压输入？")

                    # 十字探测激活：在中心点及微偏移处多次点击，确保穿透激活
                    for dx, dy in [(0,0), (2,0), (-2,0), (0,2)]:
                        await page.mouse.click(target_x + dx, target_y + dy)
                        await asyncio.sleep(0.1)

                    # 物理挤压空间：顶开可能存在的老内容
                    iprint(f"  [动作] 正在为 {kr_id} 顶开下方空间...")
                    await page.keyboard.press("Enter")
                    await page.keyboard.press("Enter")
