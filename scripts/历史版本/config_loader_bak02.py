...
class ConfigLoader:
    ...
    def _find_project_root(self) -> Path:
        ...
        if self.is_frozen:
            ...
            # ⭐ 打包环境下，项目根目录应该是 dist 的上一级
            # 例如：dist/ 的上一级是项目根目录
            parent_dir = current.parent
            if (parent_dir / "config").exists():
                print(f"[{self.script_name}] 打包环境：找到项目根目录 {parent_dir}", file=sys.stderr)
                return parent_dir
            
            # 向上查找最多3层
            for _ in range(3):
                if (current / "config").exists():
                    print(f"[{self.script_name}] 打包环境：找到项目根目录 {current}", file=sys.stderr)
                    return current
            ...
            # 默认返回当前目录的上一级
            print(f"[{self.script_name}] 打包环境：使用默认项目根目录 {self.base_dir.parent}", file=sys.stderr)
    ...
    def log_files(self) -> Dict[str, str]:
        ...
        return self.config.get("logging", {}).get("log_files", {
            "debug": "D:\\OC_DEBUG.log",
            "fatal": "D:\\FATAL_DEBUG.log",
            "error": "D:\\OC_FATAL_ERROR.log"
        })
...
class Logger:
    ...
    def __init__(self, config=None, debug=None, log_files=None):
        ...
        if log_files:
            ...
            self.log_files = {
                "debug": r"D:\OC_DEBUG.log",
                "fatal": r"D:\FATAL_DEBUG.log",
                "error": r"D:\OC_FATAL_ERROR.log"
            }
    ...
    def debug(self, message: str):
        ...
        if self.debug_mode:
            ...
            self._write_file(self.log_files.get("debug", r"D:\OC_DEBUG.log"), msg)
    ...
    def error(self, message: str):
        ...
        self._write_file(self.log_files.get("error", r"D:\OC_FATAL_ERROR.log"), msg)
        print(f"[ERROR] {message}", flush=True)

    def fatal(self, message: str):
        """致命错误日志"""
        msg = self._format_message("FATAL", message)
        self._write_file(self.log_files.get("fatal", r"D:\FATAL_DEBUG.log"), msg)
