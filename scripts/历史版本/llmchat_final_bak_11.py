...
# ==================== 4. 导入统一日志模块 ====================
print("正在导入 config_loader（含 Logger）...", file=sys.stderr)
try:
    from config_loader import ConfigLoader, Logger, create_print_functions
    print("config_loader 导入成功", file=sys.stderr)
except Exception as e:
    print(f"导入 config_loader 失败: {e}", file=sys.stderr)
    traceback.print_exc(file=sys.stderr)
    sys.exit(1)

# ==================== 5. 初始化配置 ====================
print("正在初始化 ConfigLoader...", file=sys.stderr)
try:
    config = ConfigLoader("llmchat")

    # 验证关键配置
    print(f"[调试] 项目根目录: {config.project_root}")
    print(f"[调试] 配置文件: {config.config_file}")
    print(f"[调试] llm_response_dir: {config.get_path('llm_response_dir')}")

except Exception as e:
    print(f"ConfigLoader 初始化失败: {e}", file=sys.stderr)
    traceback.print_exc(file=sys.stderr)
    sys.exit(1)

# ==================== 6. 初始化统一日志器 ====================
print("正在初始化统一日志器...", file=sys.stderr)
try:
    logger = Logger(config)
    # 创建打印函数
    dprint, iprint, safe_output = create_print_functions(debug_mode=config.debug)
    print("统一日志器初始化成功", file=sys.stderr)
except Exception as e:
    print(f"日志器初始化失败: {e}", file=sys.stderr)
    # 使用备用方案
...
try:
    ...
    print("📄 配置文件:")
    print(f"   ├─ 路径: {config.config_file}")
    print(f"   └─ 状态: {'✅ 存在' if config.config_file.exists() else '❌ 不存在'}")
    print()
    
    print("📂 关键目录:")
    print(f"   ├─ 浏览器调试目录: {config.get_path('browser_debug_dir')}")
    print(f"   ├─ 历史记录目录: {config.get_path('history_dir')}")
    print(f"   ├─ 提示词目录: {config.get_path('prompts_dir')}")
    print(f"   └─ 响应保存目录: {config.get_path('llm_response_dir')}")
    print()
    
    print("📝 提示词文件:")
    prompt_file = config.get_prompt_file("WeeklyReportOptimizePrompt.md")
    print(f"   ├─ 路径: {prompt_file}")
    print(f"   └─ 状态: {'✅ 存在' if prompt_file.exists() else '❌ 不存在'}")
    print()
    
    print("📊 周报文件:")
    # 智能查找周报文件
    report_candidates = [
        Path("团队周报_2026.3.9-3.15.json"),
        config.project_root / "团队周报_2026.3.9-3.15.json",
        Path.cwd() / "团队周报_2026.3.9-3.15.json",
    ]
    ...
    if found_report:
        print(f"   ├─ 路径: {found_report}")
        print(f"   └─ 状态: ✅ 存在")
    else:
        print(f"   ├─ 尝试路径: {report_candidates[0]}")
        print(f"   └─ 状态: ❌ 未找到（将在运行时智能查找）")
    
    print("="*60 + "\n")

except Exception as e:
    print(f"⚠️ 无法获取完整配置信息: {e}")
    print("="*60 + "\n")
...
print(f"[调试] DEBUG = {DEBUG}", file=sys.stderr)
print(f"[调试] BASE_DIR = {BASE_DIR}", file=sys.stderr)
print(f"[调试] BROWSER_STATE_FILE = {BROWSER_STATE_FILE}", file=sys.stderr)
print(f"[调试] BROWSER_DEBUG_DIR = {BROWSER_DEBUG_DIR}", file=sys.stderr)
print(f"[调试] LLMRSP_DIR = {LLMRSP_DIR}", file=sys.stderr)
...
# ==================== 导入外部浏览器自动化模块 ====================
# 尝试从 BaiduOKR_WeeklyReport 项目导入浏览器自动化模块
_BROWSER_AUTOMATION_LOADED = False
_BROWSER_AUTOMATION_PATH = None

# 查找外部模块路径
_external_module_paths = [
    # 共享库目录（推荐）
    config.project_root.parent / "libs" / "browser_automation.py",
    # 绝对路径（备用）
    Path(r"D:\OpenClaw\JobsOfAndy\libs\browser_automation.py"),
]

for _path in _external_module_paths:
    if _path.exists():
        _BROWSER_AUTOMATION_PATH = _path
        break

if _BROWSER_AUTOMATION_PATH:
    try:
        print(f"[浏览器模块] 发现外部模块: {_BROWSER_AUTOMATION_PATH}")
        import importlib.util
        _spec = importlib.util.spec_from_file_location("browser_automation", _BROWSER_AUTOMATION_PATH)
        _ba_module = importlib.util.module_from_spec(_spec)
        _spec.loader.exec_module(_ba_module)

        # 导入类和函数
        _ExternalBrowserManager = _ba_module.BrowserManager
        _ExternalHistoryManager = _ba_module.HistoryManager
        _ExternalBaseElementFinder = _ba_module.BaseElementFinder
        _highlight_element_with_index = _ba_module.highlight_element_with_index
        _clear_all_highlights = _ba_module.clear_all_highlights
        _print_element_info = _ba_module.print_element_info
        _get_element_info = _ba_module.get_element_info
        _get_selector_for_element = _ba_module.get_selector_for_element
        _analyze_page = _ba_module.analyze_page

        _BROWSER_AUTOMATION_LOADED = True
        print(f"[浏览器模块] ✅ 外部模块加载成功")
    except Exception as e:
        print(f"[浏览器模块] ❌ 外部模块加载失败: {e}")
        import traceback
        traceback.print_exc()
else:
    print(f"[浏览器模块] ⚠️ 未找到外部模块，使用内置类")
...
# ==================== 8. 平台配置 ====================
platform_bo = config.get_platform_config("BO")
...
BO = platform_bo.get("url", "https://chat.deepseek.com") if platform_bo else "https://chat.deepseek.com"
...
# ==================== 9. 打印函数（已从 config_loader 导入）====================
# 注意: dprint, iprint, safe_output 已在初始化时从 config_loader.create_print_functions() 创建
# 这里保留原始 print 的覆盖逻辑以确保兼容性
if not hasattr(builtins, '_original_print'):
    builtins._original_print = builtins.print

# ==================== 10. 安全输出函数 ====================
# safe_output 已从 config_loader 导入
...
async def wait_for_page_stable_streaming(page, timeout=300, check_interval=1.0):
    ...
    逻辑：每秒检测一次，内容长度连续5次不变则判定为稳定
    """
    print("\n[加载检测] 开始监测流式响应...")
    start_time = time.time()

    # 获取初始内容长度（直接用JS获取，更高效）
    prev_length = await page.evaluate("document.body.innerText.length")
    ...
    while not first_change and (time.time() - start_time) < max_wait_first:
        await asyncio.sleep(check_interval)
        current_length = await page.evaluate("document.body.innerText.length")
    ...
    if not first_change:
        print("  [警告] 未检测到首次内容变化，继续监测...")

    # ===== 第二阶段：持续监测直到连续5次稳定 =====
    no_change_count = 0
    stable_required = 5  # 连续5次（即5秒）无变化才算稳定
    ...
    while True:
        ...
        if time.time() - last_log_time >= 10:
            print(f"  [等待中] 已加载 {elapsed:.1f}秒, 内容长度: {current_length}, 稳定次数: {no_change_count}/{stable_required}")
            last_log_time = time.time()
        ...
        if current_length != prev_length:
            ...
            if no_change_count % 2 == 0:
                print(f"  [稳定检测] 第{no_change_count}/{stable_required}次稳定 (长度: {current_length})")
...
# ==================== 原版轮询函数（备用）====================
async def wait_for_page_stable_streaming_legacy(page, timeout=300, check_interval=1.0):
    """
    针对流式输出的智能加载检测 - 原版（轮询方式，备用）
    """
    print("\n[加载检测] 开始监测流式响应...")
    start_time = time.time()
...
async def wait_for_any_selector(page, selectors, timeout=5000, state='visible'):
    """
    等待任意一个选择器出现（并行等待）
    返回: (element, matched_selector) 或 (None, None)
    """
    async def wait_for_one(selector):
        try:
    ...
    # 并行等待所有选择器
    tasks = [asyncio.create_task(wait_for_one(s)) for s in selectors]

    # 返回第一个成功的结果
    for coro in asyncio.as_completed(tasks):
        elem, selector = await coro
        if elem:
            # 取消其他任务
            for t in tasks:
                if not t.done():
...
async def wait_for_element_stable(page, selector, stable_time=1.0, timeout=30000):
    """
    等待元素位置稳定（不再变化）
    用于处理动态加载的元素
    """
    try:
        ...
        await page.wait_for_function(
            f"""selector => {{
                ...
                if (!window.__stablePositions) window.__stablePositions = new Map();

                const rect = el.getBoundingClientRect();
                const key = `${{rect.x}}_${{rect.y}}`;

                if (window.__lastPos === key) {{
                    return true;  // 位置稳定
                }}

                window.__lastPos = key;
                return false;
            }}""",
            selector,
            timeout=timeout
        ...
        iprint(f"[等待] 元素稳定检测失败: {e}")
        return None


async def smart_wait_for_response(page, timeout=300):
    """
    智能等待响应完成（结合多种检测方法）
    """
    start_time = time.time()
    ...
    try:
        await page.wait_for_function(
            """() => {
                // 检查是否有加载/动画元素
                const loading = document.querySelector('.loading, .streaming, [data-loading="true"], .typing-indicator');
                if (loading) return false;

                // 检查是否有复制按钮（通常在响应完成后出现）
                const copyBtn = document.querySelector('button[aria-label*="复制"], button[aria-label*="Copy"], .copy-button');
                return copyBtn !== null;
            }""",
    ...
    result, _, _ = await wait_for_page_stable_streaming(page, timeout=remaining_timeout)
    return result
...
def save_clipboard_to_file(content, platform_name, input_preview):
    ...
    if formatted_content != content:
        iprint(f"[剪贴板] 格式化内容：\n"
            f"字符长度: {len(content)} => {len(formatted_content)} \n"
            f"内容:{formatted_content}")
...
if _BROWSER_AUTOMATION_LOADED:
    ...
    class BrowserManager(_ExternalBrowserManager):
        ...
        async def launch_new_browser(self, url=None):
            """兼容旧接口"""
            return await self.launch_new(url)

    class HistoryManager(_ExternalHistoryManager):
        """历史记录管理器适配器"""
        def __init__(self, name, platform=None):
            super().__init__(name=name, platform=platform, config=config, base_dir=str(config.project_root) if hasattr(config, 'project_root') else None)

    class BaseElementFinder(_ExternalBaseElementFinder):
        """元素查找器适配器"""
        def __init__(self, page, platform_name, element_name, history_manager):
            super().__init__(page, platform_name, element_name, history_manager)

    highlight_element_with_index = _highlight_element_with_index
    clear_all_highlights = _clear_all_highlights
    print_element_info = _print_element_info
    get_element_info = _get_element_info
    get_selector_for_element = _get_selector_for_element
    analyze_page = _analyze_page
    ...
    class BrowserManager:
        ...
        def get_chrome_path(self): return config.get_chrome_path()
        ...
        async def launch_new_browser(self, url=None):
            ...
            if self.headless: cmd.extend(["--headless=new", "--disable-gpu", "--no-sandbox", "--disable-dev-shm-usage"])
        ...
        async def close(self, keep_browser=True):
            if self.playwright:
                ...
                if not keep_browser:
                    ...
                    if os.path.exists(BROWSER_STATE_FILE): os.remove(BROWSER_STATE_FILE)

    class BaseElementFinder:
        """通用元素查找器"""
        def __init__(self, page, platform_name, element_name, history_manager):
            self.page, self.platform_name, self.element_name, self.history = page, platform_name, element_name, history_manager

        async def safe_find(self, coro, error_msg="操作失败"):
            try: return await coro
            except Exception as e:
                if any(k in str(e).lower() for k in ['target closed', 'page closed', 'browser closed']): raise asyncio.CancelledError()
                return None

        async def find_by_history(self, selectors, validator_func, search_radius=50):
            try: await self.page.evaluate('1')
            except: return None
            coord = self.history.get_best_coord()
            ...
            for sel in selectors:
                for el in await self.page.locator(sel).all():
                    ...
                    if box and abs(box['x']+box['width']/2-tx) < search_radius and abs(box['y']+box['height']/2-ty) < search_radius:
                        ok, meta = await validator_func(el)
                        if ok: self.history.save(box['x'], box['y'], meta); return el
            return None

        async def find_by_area(self, cx, cy, selectors, validator_func, radius=100):
            cands = []
            for sel in selectors:
                for el in await self.page.locator(sel).all():
                    if not await el.is_visible(): continue
                    box = await el.bounding_box()
                    if not box: continue
                    ex, ey = box['x']+box['width']/2, box['y']+box['height']/2
                    if abs(ex-cx) <= radius and abs(ey-cy) <= radius:
                        ok, meta = await validator_func(el)
                        if ok: cands.append({'el':el,'box':box,'meta':meta,'d':abs(ex-cx)+abs(ey-cy)})
            if cands:
                cands.sort(key=lambda x: x['d'])
                self.history.save(cands[0]['box']['x'], cands[0]['box']['y'], cands[0].get('meta'))
                return cands[0]['el']
            return None
    ...
    class HistoryManager:
        """历史记录管理器"""
        def __init__(self, name, platform=None):
            self.name, self.platform, self.history = name, platform, []
            self.cached_element, self.cached_box = None, None
            self.max_records = 5
            self.history_file = None
            try:
                if hasattr(config, 'get_history_file'): self.history_file = config.get_history_file(name, platform)
                if self.history_file:
                    self.history_file.parent.mkdir(parents=True, exist_ok=True)
                    if self.history_file.exists():
                        with open(self.history_file, 'r', encoding='utf-8') as f:
                            data = json.load(f)
                            if isinstance(data, list): self.history = sorted(data, key=lambda r: -r.get('timestamp', 0))[:self.max_records]
            except: pass

        def save(self, x, y, metadata=None):
            self.history.append({'x':x,'y':y,'timestamp':time.time(),'date':datetime.now().strftime('%Y-%m-%d %H:%M:%S'),'metadata':metadata or {}})
            self.history = sorted(self.history, key=lambda r: -r['timestamp'])[:self.max_records]
            if self.history_file:
                try:
                    with open(self.history_file, 'w', encoding='utf-8') as f: json.dump(self.history, f, indent=2, ensure_ascii=False)
                except: pass

        def get_best_coord(self): return (self.history[0]['x'], self.history[0]['y']) if self.history else None
        def get_best_coord_with_metadata(self): return {'x':self.history[0]['x'],'y':self.history[0]['y'],'metadata':self.history[0].get('metadata',{})} if self.history else None
        def extract_stable_features(self): return {}
        def update_cache(self, el, box): self.cached_element, self.cached_box = el, box
        async def get_cached(self, page, tol=10):
            if self.cached_element:
                try:
                    if await page.evaluate("el=>el.isConnected", self.cached_element):
                        nb = await self.cached_element.bounding_box()
                        if nb and abs(nb['x']-self.cached_box['x']) < tol: return self.cached_element
                except: pass
            return None
        def clear_cache(self): self.cached_element = self.cached_box = None
        ...
        try:
            ...
            for i, btn_info in enumerate(area_buttons[:max_buttons]):
                ...
                if is_copy:
                    ...
                    # if DEBUG:
                    #     resp = input(f"\n 调试模式{DEBUG}   是否使用此按钮？(y/n/q-退出): ").strip().lower()
                    #     if resp == 'y':
                    #         return button
                    #     elif resp == 'q':
                    #         return None
...
async def click_copy_button_and_get_content(page, copy_button):
    ...
    try:
        ...
        if clipboard_content:
            ...
            # preview = clipboard_content[:500].replace('\n', ' ').replace('\r', '')
...
async def main():
    ...
    platform_group.add_argument('--BO', '-BO', action='store_true', help='使用百度ONEAPI平台(默认)')
    platform_group.add_argument('--DS', '-DS', action='store_true', help='使用DeepSeek平台 ')
    ...
    if use_file_upload:
        ...
        if platform_name == "DeepSeek" and hasattr(handler, 'upload_file_with_filechooser'):
            ...
            print(f"[复制操作] 查找结果: {'找到' if copy_button else '未找到'}")
