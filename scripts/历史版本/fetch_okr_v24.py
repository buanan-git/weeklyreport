...
def integrate_weekly_reports_v10():
    ...
    # 生成JSON格式的团队周报
    team_report = generate_team_weekly_v10(reports)
    
    # 保存为JSON文件（替代原来的Markdown）
...
def generate_team_weekly_v10(reports):
    """
    V10生成团队周报 - JSON格式（类似个人周报）
    优化：
    1. 输出JSON格式，便于后续处理
    2. OKR标题去重：去掉重复的 O1/O2/O3/KR1/KR2/KR3 字样，但保留一个
    """
    ...
    team_report = {
        ...
        'content': {
            ...
            'part2_okr_structure': optimized_okr_structure,  # 使用去重后的OKR结构
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        if integrate_result:
            ...
            print(f"  - team_weekly_report_v10.json (V10团队周报JSON格式)")
            print(f"  - team_weekly_report_v10.md (V10团队周报Markdown格式)")
