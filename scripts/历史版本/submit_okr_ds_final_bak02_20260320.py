...
async def fast_type_text(page, text, element=None):
    ...
    try:
        ...
        print(f"  [fast_type] execCommand失败，使用快速type: {e}")
        try:
            await page.keyboard.type(text, delay=0)
            return True
        except Exception as e2:
            print(f"  [fast_type] 快速type也失败: {e2}")
            return False
...
if _BROWSER_AUTOMATION_LOADED:
    ...
    class BrowserManager:
        ...
        def _get_available_port(self):
            """获取可用端口"""
            import random
            ports = list(range(9223, 9300))
            random.shuffle(ports)
            for port in ports:
                if not self._is_port_in_use(port):
                    return port
            return random.randint(9223, 9299)
...
def parse_json_file(json_path):
    ...
    try:
        ...
        print(f"  后150字符: {raw_content[-150:]}")
    except Exception as e:
            ...
            try:
                ...
                if match:
                    ...
                    if content_str.endswith('"}'):
                        print(f"  [OK] 找到完整的 content 字符串")
                        # 尝试解析 content 字符串
                        try:
                            inner_data = json.loads(content_str)
                            print(f"  [OK] 成功解析 content 字符串")
                            
                            # 构造完整的数据
                            data = {"content": inner_data}
                            print(f"  [OK] 已修复 JSON 数据")
                        except json.JSONDecodeError as e2:
                            print(f"  [ERROR] content 字符串解析失败: {e2}")
                            return None
                    else:
                        print(f"  [WARN] content 字符串可能被截断")
                        # 尝试补全
                        ...
                        try:
                            ...
                            print(f"  [ERROR] 补全后仍然解析失败")
                            return None
            except Exception as e2:
                print(f"  [ERROR] 修复失败: {e2}")
                return None
        else:
            return None
    ...
    if "report_type" in data and "content" in data and isinstance(data["content"], dict):
        ...
        print(f"  从 content 字典中提取数据")
    
    # 情况2: 有 content 字段，且是字符串（大模型返回的格式）
    elif "content" in data and isinstance(data["content"], str):
        print(f"\n[DEBUG] 情况2: 发现 content 字符串字段")
        content_str = data["content"]
        print(f"  content字符串长度: {len(content_str)}")
        print(f"  content前100字符: {content_str[:100]}")
        print(f"  content后100字符: {content_str[-100:]}")
        
        try:
            # 解析 content 字符串为 JSON
            inner_data = json.loads(content_str)
            print(f"  [OK] 成功解析content字符串为JSON")
            print(f"  解析后的内层键: {list(inner_data.keys())}")
            
            # 检查内层是否有 content 字典
            if "content" in inner_data and isinstance(inner_data["content"], dict):
                print(f"  [INFO] 从内层JSON中提取 content 字典")
                final_content = inner_data["content"]
            else:
                print(f"  [WARN] 内层没有 content 字典，直接使用内层数据")
    ...
    if not isinstance(final_content, dict):
        print(f"  [ERROR] final_content 不是字典类型: {type(final_content)}")
        return None
    ...
    if "part1_work_summary" in final_content:
        ...
        print(f"  可用键: {list(final_content.keys())}")
...
async def scroll_container(page, selector, center):
    ...
    for i in range(15):  # 优化：30次 → 15次（每次滚屏更多）
        ...
        if i % 5 == 0:
            print(f"{i*800}px", end=" ", flush=True)
    print("OK")
...
async def fill_part1_work_summary(page, part1_data):
    ...
    try:
        ...
        print(f"  调试信息: {title_info.get('debug', 'N/A')}")
        ...
        if realtime_info.get('success'):
            print(f"  实时定位坐标: ({realtime_info['viewX']:.1f}, {realtime_info['viewY']:.1f}) [视口]")
            print(f"  页面绝对坐标: ({realtime_info['clickX']:.1f}, {realtime_info['clickY']:.1f})")
            print(f"  调试信息: {realtime_info.get('debug', 'N/A')}")
        ...
        if realtime_info.get('success'):
            click_x = realtime_info['clickX']
            click_y = realtime_info['clickY']
            print(f"  实时定位坐标: ({click_x:.1f}, {click_y:.1f})")
        else:
            print(f"  [⚠️] 实时定位失败，使用之前保存的坐标")

        print(f"  [INFO] 最终点击坐标: ({click_x:.1f}, {click_y:.1f})")
...
async def fill_okr_section(page, part2_data):
    ...
    try:
        ...
        for order_idx, kr_info in enumerate(fill_order):
            ...
            print("\n    [输入内容] 开始输入...")
            # 一次性粘贴全部内容
            print(f"      内容长度: {len(weekly_work)} 字符")
...
async def main():
    ...
    print(f"\n{'='*60}")
    print("[系统] 初始化浏览器管理器...")
    print(f"[系统] 外部模块加载状态: {_BROWSER_AUTOMATION_LOADED}")
    ...
    print(f"[浏览器] 状态文件: {BROWSER_STATE_FILE}")
    print(f"[浏览器] 状态文件存在: {os.path.exists(str(BROWSER_STATE_FILE))}")

    if browser_manager.load_state():
        print(f"[浏览器] 加载状态成功: 端口={browser_manager.cdp_port}, PID={browser_manager.browser_pid}")
        browser_connected = await browser_manager.connect_to_existing_browser()
        if not browser_connected:
            print("[浏览器] 连接已存在的浏览器失败")
    else:
        print("[浏览器] 加载状态失败或浏览器进程不存在")
