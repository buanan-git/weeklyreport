...
def generate_team_weekly_v10(reports):
    """V10生成团队周报 - 统一格式汇总（以s673090为基准结构，合并本周工作内容）"""
    lines = [
    ...
    for report in reports:
        ...
        staff_id = report.get('id', 'unknown')
        ...
        for o in okr_structure:
            ...
            for kr in o.get('key_results', []):
                ...
                if weekly_work:
                    debug_stats['weekly_work收集'].append(f"{staff_id}/{o_id}/{kr_id}: {len(weekly_work)}条")
    ...
    # 记录weekly_work_map构建后的统计
    for o_id, krs in weekly_work_map.items():
        debug_stats['weekly_work_map构建后'][o_id] = {}
        for kr_id, works in krs.items():
            debug_stats['weekly_work_map构建后'][o_id][kr_id] = len(works)
    ...
    if base_okr_structure:
        ...
        for o in base_okr_structure:
            ...
            for kr in o.get('key_results', []):
                ...
                # 记录原始数量
                original_count = len(weekly_work)

                # 过滤"Hi卜安安同学"及后续内容
                weekly_work = filter_hi_content(weekly_work)
                after_filter_count = len(weekly_work)

                # 去重（提高阈值以保留更多内容）
                weekly_work = deduplicate_lines(weekly_work, 0.85)
                after_dedup_count = len(weekly_work)

                part2_stats.append(f"{o_id}/{kr_id}: 原始{original_count}条 -> 过滤后{after_filter_count}条 -> 去重后{after_dedup_count}条")
        ...
        lines.append("## 调试信息")
        lines.append("")
        lines.append("### weekly_work收集统计")
        lines.append("")
        for stat in debug_stats['weekly_work收集']:
            lines.append(f"- {stat}")
        lines.append("")
        lines.append("### weekly_work_map构建后统计")
        lines.append("")
        for o_id, krs in debug_stats['weekly_work_map构建后'].items():
            for kr_id, count in krs.items():
                lines.append(f"- {o_id}/{kr_id}: {count}条")
        lines.append("")
        lines.append("### 第二部分处理统计")
        lines.append("")
        for stat in part2_stats:
            lines.append(f"- {stat}")
        lines.append("")
