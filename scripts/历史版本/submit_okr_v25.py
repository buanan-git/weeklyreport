...
class BrowserManager:
    ...
    async def launch_new_browser(self, headless=False):
        ...
        try:
            ...
            # 保存状态
            self.browser_pid = self.browser.process.pid if hasattr(self.browser, 'process') else None
            self.save_state()
            iprint("[浏览器] 新浏览器启动成功")
            return True
        except Exception as e:
    ...
    async def close(self, keep_browser=True):
        ...
        if self.playwright:
            if keep_browser:
                ...
                iprint("[浏览器] 已断开连接，浏览器进程继续运行，可下次复用")
                iprint(f"[浏览器] 如需重新连接，端口: {self.cdp_port}")
            else:
                # 关闭浏览器和连接
                if self.browser:
...
class BrowserManager:
    ...
    def save_state(self):
        ...
        if self.cdp_port:
            ...
            try:
                with open(BROWSER_STATE_FILE, "w", encoding="utf-8") as f:
                    json.dump(state, f)
                print(f"[浏览器] 状态已保存到 {BROWSER_STATE_FILE}")
            except Exception as e:
                print(f"[浏览器] 保存状态失败: {e}")
    ...
    def load_state(self):
        ...
        if os.path.exists(BROWSER_STATE_FILE):
            try:
                ...
                self.cdp_port = state.get("cdp_port")
                print(f"[浏览器] 从文件加载状态成功: 端口={self.cdp_port}")
                return True
            except Exception as e:
                print(f"[浏览器] 加载状态失败: {e}")
        return False
    ...
    async def connect_to_existing_browser(self):
        ...
        try:
            ...
            if contexts:
                ...
                if pages:
                    self.page = pages[0]
                    print("[浏览器] 成功连接到已存在的浏览器")
                    return True
                else:
                    self.page = await self.context.new_page()
                    return True

            print("[浏览器] 未找到现有的context")
            return False
        except Exception as e:
            print(f"[浏览器] 连接失败: {e}")
            return False
    ...
    async def launch_new_browser(self, headless=False):
        ...
        self.cdp_port = random.randint(9222, 9299)
        print(f"[浏览器] 启动新浏览器，调试端口: {self.cdp_port}")
    ...
    async def get_or_create_page(self):
        ...
        # 启动新浏览器
        return await self.launch_new_browser(headless=False)

    async def close(self, keep_browser=True):
        """关闭连接，可选择是否保留浏览器"""
        if self.playwright:
            if keep_browser and self.browser:
                # 只断开连接，不关闭浏览器
                await self.playwright.stop()
                print("[浏览器] 已断开连接，浏览器继续运行，可下次复用")
                print(f"[浏览器] 如需重新连接，端口: {self.cdp_port}")
            else:
                # 关闭浏览器
                if self.browser:
                    await self.browser.close()
