...
MY_ID = 's673090'
STAFF_IDS = [
    's801573', 's733504', 's682083', 's929270', 's798268','s811385', 's865585', 's783729', 's400192', 's823700'
]
...
def integrate_weekly_reports_v10():
    ...
    # 保存JSON文件
    json_file = 'team_weekly_report_v10.json'
    with open(json_file, 'w', encoding='utf-8') as f:
    ...
    md_file = 'team_weekly_report_v10.md'
...
def generate_team_weekly_v10(reports):
    ...
    # ===== 修复：定义 optimized_okr_structure =====
    # 构建去重后的OKR结构（去掉重复的O1/O2/O3/KR1/KR2/KR3字样，但保留一个）
    optimized_okr_structure = []
    for o in base_okr_structure:
        o_id = o.get('id', '')
        o_title_raw = o.get('title', '')
        # 去重：去掉重复的O1/O2/O3字样，保留一个
        o_title_clean = clean_okr_title(o_title_raw, o_id)
        
        optimized_krs = []
        for kr in o.get('key_results', []):
            kr_id = kr.get('id', '')
            kr_title_raw = kr.get('title', '')
            # 去重：去掉重复的KR1/KR2/KR3字样，保留一个
            kr_title_clean = clean_okr_title(kr_title_raw, kr_id)
            ...
            optimized_krs.append({
                'id': kr_id,
                'title': kr_title_clean,  # 使用去重后的标题（保留一个标记）
                'weekly_work': weekly_work
            })
        
        optimized_okr_structure.append({
            'id': o_id,
            'title': o_title_clean,  # 使用去重后的标题（保留一个标记）
    ...
    team_report = {
        ...
        'members': members,  # 保持传入顺序
        'content': {
            'part1_work_summary': all_part1,
            'part2_okr_structure': optimized_okr_structure,  # 使用修复后的变量
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        print(f"\n{'='*60}")
        print("整合团队周报...")
        print(f"  配置的成员顺序 ({len(STAFF_IDS)} 人):")
        for i, staff_id in enumerate(STAFF_IDS, 1):
            print(f"    {i:2d}. {staff_id}")
        ...
        if integrate_result:
            ...
            print(f"{'='*60}")
            print(f"\n生成的文件:")
            print(f"  - 姓名_日期_周报.json (个人周报)")
            print(f"  - team_weekly_report_v10.json (团队周报 - 按配置顺序)")
            print(f"  - team_weekly_report_v10.md (团队周报Markdown)")
            print(f"  - personal_reference_v10.md (个人参考)")
