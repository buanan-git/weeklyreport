...
async def fill_part1_work_summary(page, part1_data):
    ...
    try:
        # 先尝试精确匹配
        print(f"  [定位] 尝试精确匹配 '{target_label}'...")
        label_locator = page.get_by_text(target_label, exact=True).first

        if await label_locator.count() > 0 and await label_locator.is_visible(timeout=3000):
            print(f"    [OK] 找到精确匹配的标签")

            # 查找该标签下的输入框
            # 策略：先向上查找容器，再查找textarea
            parent = label_locator.locator('xpath=ancestor::div[contains(@class, "ant-form-item") or contains(@class, "form-item") or contains(@class, "section")]').first

            if await parent.count() == 0:
                # 如果没找到特定容器，尝试通用父容器
                parent = label_locator.locator('xpath=ancestor::div[2]').first

            if await parent.count() > 0:
                # 在容器内查找输入框
                textarea = parent.locator('textarea, input[type="text"], [contenteditable="true"]').first

                if await textarea.count() > 0 and await textarea.is_visible():
                    await textarea.fill(content)
                    print(f"    [OK] 已在 '本周工作总结' 区域填写内容")
                    input_filled = True
                    return True

        # 策略2: 如果精确匹配失败，尝试模糊匹配
        if not input_filled:
            print(f"  [定位] 精确匹配失败，尝试模糊匹配...")
            variants = ["本周工作总结", "工作总结", "本周工作"]

            for variant in variants:
                try:
                    label = page.get_by_text(variant, exact=False).first
                    if await label.count() > 0 and await label.is_visible(timeout=2000):
                        print(f"    [OK] 找到模糊匹配: '{variant}'")
                        ...
                        if for_attr:
                            ...
                            if await input_by_id.count() > 0:
                                await input_by_id.fill(content)
                                print(f"    [OK] 通过for属性填写")
                                return True

                        # 方法2: 查找相邻的输入框
                        parent = label.locator('xpath=ancestor::div[2]').first
                        if await parent.count() > 0:
                        ...
                        # 方法3: 直接查找页面中所有可见的textarea，选择最相关的一个
                        all_textareas = await page.locator('textarea:visible').all()
                        if all_textareas:
                            # 选择与标签距离最近的一个（简化：选择第一个）
                            await all_textareas[0].fill(content)
                            print(f"    [OK] 填写到第一个可见文本框")
                            return True

                except Exception as e:
                    print(f"    [!] 尝试 '{variant}' 失败: {e}")
                    continue

        # 策略3: 如果都找不到，创建新区域
        if not input_filled:
            print(f"  [策略3] 未找到现有区域，尝试创建 '本周工作总结' 区域...")
            return await create_section_with_content(page, "本周工作总结", content)
