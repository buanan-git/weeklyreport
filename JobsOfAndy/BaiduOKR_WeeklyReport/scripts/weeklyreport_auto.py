...
# ==================== 设置 UTF-8 编码 ====================
# 注意：PyInstaller GUI模式下 sys.stdout/stderr 可能为 None 或无 buffer 属性
if sys.platform == 'win32':
    import os
    os.environ["PYTHONUTF8"] = "1"
    # 安全地设置控制台编码，避免在 GUI 模式下出错
    for stream_name in ['stdout', 'stderr']:
        stream = getattr(sys, stream_name, None)
        if stream is not None:
            try:
                # 检查是否有 buffer 属性和 encoding 属性
...
# ==================== 模块导入策略 ====================
# 1. config_loader 从当前项目目录加载（读取项目特定配置）
# 2. llmchat_final, llmapi_v10 从 TokenLess/scripts 加载（共享模块）
scripts_dir = Path(__file__).parent.absolute()
tokenless_scripts_dir = Path(__file__).parent.parent.parent / "TokenLess" / "scripts"

# 当前目录优先（用于 config_loader）
...
def _update_progress_file(step, step_name, message="", detail=None, status="running"):
    """更新进度文件（供主程序读取）"""
    _init_progress_file()
    ...
    try:
        ...
        if PROGRESS_FILE.exists():
            ...
            data = {"details": [], "elapsed": 0, "total_steps": 4}

        # 步骤切换时清空旧的 details，避免前端滞后显示上一步的输出
...
# API 模式使用专用 prompt（强制 JSON 输出）
PROMPT_FILE_API = Path(PROMPT_FILE).parent / "WeeklyReportOptimizePrompt_API.md"

# ==================== 添加共享模块路径 ====================
# TokenLess/scripts 用于 llmchat_final, llmapi_v10
if str(tokenless_scripts_dir) not in sys.path:
    sys.path.insert(0, str(tokenless_scripts_dir))
    if DEBUG:
        print(f"[weeklyreport] 添加共享模块路径: {tokenless_scripts_dir}")
...
async def step_fetch_data():
    ...
    _update_progress_file(1, "读取周报", "正在获取团队周报数据...", "启动浏览器")
    ...
    try:
        ...
        if not original_data:
            log_error("获取到的数据为空")
            _update_progress_file(1, "读取周报", "获取数据失败", "数据为空", "error")
            return None

        log_success(f"团队周报数据获取成功")
        _update_progress_file(1, "读取周报", "数据获取成功", f"成功获取周报数据")
        ...
        if _is_browser_disconnected(e):
            log_error("浏览器连接断开，重连失败将自动退出")
            _update_progress_file(1, "读取周报", "浏览器断开", "连接已断开", "error")
            return None
...
async def step_optimize_report(original_data):
    """步骤2&3: 整合并优化周报"""
    log_info("开始执行步骤2: 整合周报数据")
    _update_progress_file(2, "整合周报", "正在准备数据...", "创建临时文件")
    ...
    try:
        ...
        _update_progress_file(2, "整合周报", "数据整合完成", "开始调用大模型")
        ...
        if LLM_CALL_TYPE == "web":
            ...
            _update_progress_file(3, "优化周报", "正在调用大模型优化...", f"平台: {LLM_PLATFORM}, 模式: web")
            ...
            _update_progress_file(3, "优化周报", "正在调用API优化...", f"平台: {LLM_PLATFORM}, 模式: api")
            optimized_data = await llm_api_resp(
                attachment_file=temp_attachment,
                platform=LLM_PLATFORM,
                prompt_file=str(PROMPT_FILE_API),
                timeout=300
            )

            if not optimized_data:
                log_warning("API调用失败，回退到网页自动化")
                _update_progress_file(3, "优化周报", "API失败，回退网页模式...", "回退到web模式")
                optimized_data = await llmchat_resp(
                    attachment_file=temp_attachment,
                    platform=LLM_PLATFORM,
                    prompt_file=str(PROMPT_FILE),
        ...
        if optimized_data:
            log_success("优化成功")
            _update_progress_file(3, "优化周报", "优化完成", "大模型处理成功")
...
async def step_submit_report(optimized_data):
    """步骤4: 提交优化后的周报"""
    log_info("开始执行步骤4: 提交优化后的周报")
    _update_progress_file(4, "提交周报", "正在提交周报...", "启动提交流程")

    try:
        result = await submit_okr_report(optimized_data)
        log_success(f"周报提交成功: {result}")
        _update_progress_file(4, "提交周报", "提交成功", f"结果: {result}")
        return True

    except Exception as e:
        if _is_browser_disconnected(e):
            log_error("浏览器连接断开，重连失败将自动退出")
            _update_progress_file(4, "提交周报", "浏览器断开", "连接已断开", "error")
            return False
...
async def main():
    ...
    # 初始化进度
    _update_progress_file(0, "初始化", "准备启动...", "加载配置完成")
    ...
    if original_data is None:
        ...
        _update_progress_file(1, "读取周报", "获取数据失败", status="error")
        return False
    ...
    if optimized_data is None:
        ...
        _update_progress_file(3, "优化周报", "优化失败", "大模型调用失败", status="error")
        return False
    ...
    if not await step_submit_report(optimized_data):
        ...
        _update_progress_file(4, "提交周报", "提交失败", status="error")
        return False

    print("\n" + "="*50)
    log_success("🎉 所有步骤执行完成！")
    print("="*50)
    _update_progress_file(4, "完成", "执行完成", "所有步骤执行成功", status="completed")
