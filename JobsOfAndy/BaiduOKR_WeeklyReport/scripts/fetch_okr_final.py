...
try:
    ...
    pass  # dprint("[导入] 成功加载 browser_automation 通用模块")
except ImportError as e:
    print(f"[警告] 无法导入 browser_automation 模块: {e}", flush=True)  # dprint不可用，保留print
    print("[警告] 将使用脚本内建的简化版本", flush=True)  # dprint不可用，保留print
    ...
    class BrowserManager:
                ...
                try:
                    ...
                    if pid and self._is_running(pid):
                        if port and not self._is_port_in_use(port):
                            self.cdp_port, self.browser_pid = port, pid
                            return True
                except: pass
                # 清理无效状态文件
                try:
        ...
        def _is_running(self, pid):
            ...
            try:
                if platform.system() == "Windows":
                    return str(pid) in subprocess.run(f'tasklist /FI "PID eq {pid}"', shell=True, capture_output=True, text=True, encoding='gbk', errors='ignore').stdout
                os.kill(pid, 0)
                return True
            except: return False
        ...
        async def launch_new(self, url=None):
            ...
            cmd = [CHROME_PATH, f"--remote-debugging-port={self.cdp_port}", f"--user-data-dir={os.path.abspath(BROWSER_DEBUG_DIR)}", "--no-first-run", "--no-default-browser-check",
                   "--disable-extensions", "--disable-background-networking", "--disable-default-apps", "--disable-sync", "--disable-translate",
                   "--metrics-recording-only", "--mute-audio", "--no-sandbox",
            ...
            try:
                ...
                await asyncio.sleep(3)  # 初始等待3秒，给Chrome充分启动时间
                self.playwright = await async_playwright().start()
                for i in range(5):  # 5次重试
                    try:
                        ...
                        wait_time = 2 + i  # 递增等待: 2s, 3s, 4s, 5s, 6s
                        if i < 4:
        ...
        async def ensure_page(self):
            dprint("[DEBUG] ensure_page: 开始")
            if not self.browser:
                dprint("[DEBUG] ensure_page: browser 为空")
                return False
            dprint("[DEBUG] ensure_page: browser 存在")
            try:
                self.context = self.browser.contexts[0] if self.browser.contexts else await self.browser.new_context()
                dprint("[DEBUG] ensure_page: context 获取成功")
            except Exception as e:
                dprint(f"[DEBUG] ensure_page: 获取context异常: {e}")
                return False
            try:
                self.page = self.context.pages[0] if self.context.pages else await self.context.new_page()
                dprint("[DEBUG] ensure_page: page 获取成功")
            except Exception as e:
                dprint(f"[DEBUG] ensure_page: 获取page异常: {e}")
                return False
            return True
...
if platform.system() == "Windows":
    ...
    CHROME_PATH = _mac_chrome if os.path.exists(_mac_chrome) else (shutil.which("chromium") or "google-chrome")
else:
    # Linux
...
def parse_weekly_report_structure_v10(full_text):
    ...
    V10核心：解析三部分结构（增强过滤版）
    """
    # 全局过滤：如果检测到"HiXXX同学"欢迎语，抛弃后续所有内容
    import re as _re
    hi_match = _re.search(r'Hi.{1,10}同学', full_text)
    if hi_match:
        full_text = full_text[:hi_match.start()]
    ...
    for i, line in enumerate(lines):
        if 'OKR' in line.upper() and '的' in line:
            ...
            dprint(f"提取OKR标题和姓名：{result['okr_title']},{name}")
            if name:
    ...
    while i < len(lines):
        ...
        if '本周工作总结' in line:
            if first_summary_found:
                dprint(f"    [V10] 检测到第二个'本周工作总结'，第一部分内容获取完成")
                break
            first_summary_found = True
            dprint(f"    [V10] 找到第一个'本周工作总结'，开始提取内容")
            i += 1
            while i < len(lines):
                content_line = lines[i]
                # 如果遇到第二个"本周工作总结"，停止
                if '本周工作总结' in content_line:
                    dprint(f"    [V10] 检测到第二个'本周工作总结'，停止提取")
                    break
                # 如果遇到OKR标题或O/KR标题，停止
                ...
                if len(content_line) > 5 and is_valid_content(content_line) and not is_title_line(content_line):
                    dprint(f"      [添加] 有效内容: {content_line[:80]}...")
    ...
    if okr_header_index >= 0:
        ...
        while i < len(okr_lines):
            ...
            if part3_pattern.search(line):
                dprint(f"    [V10] 检测到第三部分标题，停止OKR解析")
                break
            ...
            if o_match:
                ...
                dprint(f"    [V10] O{o_num}标题(保留): {o_title[:100]}")
            ...
            if kr_match and current_o:
                ...
                dprint(f"    [V10] KR{kr_num}标题(保留): {kr_title[:100]}")
            ...
            if weekly_match and current_kr:
                ...
                if content_after and is_valid_content(content_after) and not is_title_line(content_after):
                    dprint(f"    [V10] 本周工作(首行): {content_after[:80]}")
                ...
                while i < len(okr_lines):
                    ...
                    if len(next_line.strip()) > 1 and is_valid_content(next_line) and not is_title_line(next_line):
                        dprint(f"    [V10] 本周工作(续): {next_line.strip()[:80]}")
        ...
        dprint(f"\n    [V10] 解析完成摘要:")
        for o in result['part2_okr_structure']:
            dprint(f"      {o['id']}: {o['title'][:80]}")
            for kr in o.get('key_results', []):
                dprint(f"        {kr['id']}: title={kr['title'][:60]}, work_items={len(kr['weekly_work'])}")
            ...
            if keyword in line:
                ...
                while j < len(lines):
                    ...
                    if len(content) > 5 and is_valid_content(content) and not is_title_line(content):
                        dprint(f"      [第三部分-{keyword}] 添加: {content[:80]}...")
...
def extract_name_from_okr_header_enhanced(text):
    ...
    # 打印调试信息
    dprint(f"[调试] 尝试从文本提取姓名: {text[:100]}")
    ...
    for pattern in patterns:
        ...
        if match:
            name = match.group(1)
            dprint(f"[调试] 通过模式 '{pattern}' 提取到姓名: {name}")
            return name
    
    # 如果都失败，尝试提取任何中文字符
    chinese_chars = re.findall(r'[\u4e00-\u9fa5]{2,4}', text)
    if chinese_chars:
        dprint(f"[调试] 通过中文字符提取到姓名: {chinese_chars[0]}")
...
async def find_main_content_area_exact(page):
    ...
    try:
        title_selectors = [
            "xpath=//*[contains(text(), '本周工作总结')]",
            "xpath=//*[contains(text(), 'OKR进度')]",
        ]
        summary_title = None
        for selector in title_selectors:
            summary_title = await page.query_selector(selector)
            if summary_title:
                break
                    ...
                    if class_name:
                        ...
                        for cls in classes:
                            if cls and len(cls) > 2 and not any(c.isdigit() for c in cls[-4:]):
                                selector = f".{cls}"
                                dprint(f"  找到精确内容区域 (class): {selector}")
                                return selector
                    
                    # 如果都没有，返回xpath
                    dprint(f"  找到精确内容区域 (元素)")
        ...
        try:
            ...
            if element and await element.is_visible():
                ...
                if text and ("本周工作总结" in text or "OKR" in text):
                    dprint(f"  通过CSS选择器定位成功: {selector}")
                    return selector
        except Exception:
            continue

    # 2. 基于内容特征的智能定位
    dprint("  CSS选择器失效，尝试基于内容特征定位...")
    try:
        ...
        if title_element:
            ...
            if main_content_selector:
                dprint(f"  通过内容特征定位成功: {main_content_selector}")
...
async def fetch_single_staff_v10(page, staff_id, date, desc, index=0, total=0, history_manager=None):
    ...
    try:
        ...
        try:
            ...
            # 浏览器断连时向上抛出，由调用方的 reconnect 逻辑处理
            if _is_browser_disconnected(nav_error):
                raise
            # 网络不可达（DNS解析失败等）直接返回错误，不继续后续步骤
            if _is_network_error(nav_error):
                iprint(f"\n❌ 网络错误，无法访问目标页面: {nav_error}")
                return {'id': staff_id, 'date': date, 'status': 'error', 'error': f'网络不可达: {nav_error}'}
            try:
        ...
        try:
            ...
            dprint(f"使用选择器: {main_content_selector}")
        except Exception as e:
            dprint(f"定位失败: {e}, 使用body")
            main_content_selector = "body"
            if DEBUG:
                import traceback
                traceback.print_exc()
        ...
        iprint(f"姓名:{employee_name}, 工作总结:{len(parsed['part1_work_summary'])}条, O数量:{len(parsed['part2_okr_structure'])}")

        iprint("  第三部分统计...", end=" ")
        metrics = parsed.get('part3_metrics', {})
        metrics_summary = ', '.join([f"{k}:{len(v)}条" for k, v in metrics.items()])
        iprint(metrics_summary)
        ...
        iprint(f"  ✓ 完成: {safe_name} (已保存 {json_file})")
        ...
        try:
            ...
            if page:
                ...
                iprint(f"  错误截图已保存: {screenshot_file}")
            else:
                iprint("  无法截图：page对象不存在")
        except Exception as screenshot_error:
            iprint(f"  截图失败: {screenshot_error}")
...
def integrate_weekly_reports_v10():
    ...
    iprint("\n" + "="*60)
    iprint(f'整合周报...{targetweek}_周报.json')
    iprint(f'整合周报...{week_before}_周报.json')

    reports_dict = {}
    # 读取所有周报文件到字典，方便按ID查找
    # 定义需要查找的所有文件模式（使用 WEEKLYREPORT_DIR 变量）
    patterns = [
        os.path.join(WEEKLYREPORT_DIR, f'*{targetweek}*_周报.json'),
        os.path.join(WEEKLYREPORT_DIR, f'*{week_before}*_周报.json')
    ]
    # 遍历所有匹配模式的文件
    for pattern in patterns:
            ...
            try:
                with open(file, 'r', encoding='utf-8') as f:
                    ...
                    if staff_id:
                        ...
                        iprint(f"  警告: 文件 {file} 缺少ID字段")
            except Exception as e:
                iprint(f"  警告: 无法读取 {file} - {e}")
    ...
    for staff_id in STAFF_IDS:
        if staff_id in reports_dict:
            ...
            dprint(f"  ✓ 添加: {staff_id}")
        else:
            missing_ids.append(staff_id)
            iprint(f"  ⚠️ 缺失: {staff_id}")
...
def generate_team_weekly_v10(reports):
    ...
    if not my_report:
        iprint(f"  [警告] 未找到基准员工 {MY_ID} 的周报，将使用第一个报告作为基准")
    ...
    # 先处理MY_ID：仅提取OKR结构作为框架模板，不合并具体内容
    if my_report:
        staff_id = my_report.get('id', '')
        staff_name = my_report.get('name', '')
        dprint(f"  [处理] 基准(仅框架): {staff_id} - {staff_name}")

        content = my_report.get('content', {})

        # 只初始化weekly_work_map的结构（O/KR映射），不填入MY_ID的工作内容
    ...
    for staff_id in STAFF_IDS:
        ...
        if found_report:
            ...
            dprint(f"  [处理] {staff_id} - {staff_name}")
                ...
                if o_id in weekly_work_map:
                    for kr in o.get('key_results', []):
                        ...
                        if kr_id in weekly_work_map[o_id]:
                            ...
                            dprint(f"    匹配到 {o_id} {kr_id}: 添加 {len(weekly_work)} 条工作项")
                else:
                    dprint(f"    [警告] 报告中的 {o_id} 不在基准结构中，跳过")
            
            processed_ids.add(staff_id)
        else:
            dprint(f"  [缺失] 未找到 {staff_id} 的周报")
    ...
    for report in reports:
        ...
        if staff_id not in processed_ids:
            ...
            dprint(f"  [额外] {staff_id} - {staff_name}")
    ...
    for o in base_okr_structure:
        ...
        for kr in o.get('key_results', []):
            ...
            dprint(f"    [合并] {o_id} {kr_id}: 共 {len(weekly_work)} 条工作项")
...
def _is_network_error(error):
    """检测异常是否为网络不可达（DNS解析失败、连接超时等致命网络错误）"""
    err_str = str(error).lower()
    network_keywords = [
...
async def _wait_for_login(page, timeout=300):
    """等待用户完成登录，轮询检测 URL 变化。返回 True=登录成功, False=超时"""
    iprint("\n" + "="*60)
    ...
    while True:
        try:
            current_url = page.url
            dprint(f"[DEBUG] 检测登录状态... URL: {current_url}")
...
async def _is_page_unreachable(page):
    """检测页面是否显示网络错误（无法访问此网站、DNS解析失败等）"""
    try:
        # 检查页面标题和内容是否包含错误标识
        title = await page.title()
        title_lower = title.lower() if title else ""
        # Chrome 错误页面的标题通常含有目标域名或为空
...
async def fetch_okr_data(weeks_ago=0):
    ...
    # 创建浏览器管理器
    dprint(f"[浏览器] 外部模块加载状态: {USE_AUTOMATION_MODULE}")
    ...
    # 确保页面实例存在
    dprint("[DEBUG] 调用 ensure_page()...")
    try:
        page_result = await browser_manager.ensure_page()
        dprint(f"[DEBUG] ensure_page() 返回: {page_result}")
    except Exception as e:
        dprint(f"[DEBUG] ensure_page() 异常: {e}")
        if DEBUG:
            import traceback
            traceback.print_exc()
        return None
    ...
    if not page:
        dprint("[DEBUG] page 为空，退出")
        return None
    dprint(f"[DEBUG] page 实例获取成功，当前URL: {page.url}")

    # 获取当前URL并判断是否需要初始导航
    dprint("[DEBUG] 获取当前URL...")
    try:
        current_url = page.url
        dprint(f"[DEBUG] 当前URL: {current_url}")
    except Exception as e:
        dprint(f"[DEBUG] 获取URL异常: {e}")
        current_url = ""
    
    if BASE_URL not in current_url:
        iprint(f"\n[导航] 当前URL: {current_url}，导航到目标页面...")
        dprint(f"[DEBUG] 开始 page.goto({BASE_URL})...")
        try:
            await page.goto(BASE_URL, wait_until='domcontentloaded', timeout=30000)
            dprint("[DEBUG] page.goto() 成功")
        except asyncio.TimeoutError:
            iprint("[!] 页面加载超时，重试一次...")
            dprint("[DEBUG] 重试 page.goto()...")
            await page.goto(BASE_URL, wait_until='domcontentloaded', timeout=30000)
            dprint("[DEBUG] 重试成功")
        except Exception as e:
        ...
        await asyncio.sleep(5)
        current_url = page.url
        dprint(f"[DEBUG] 导航后URL: {current_url}")
    ...
    # 新增：页面内容检测（URL判断可能漏掉的场景，如UUAP未重定向）
    if not need_login:
        try:
            has_login_form = await page.evaluate("""() => {
                const hasPasswordInput = !!document.querySelector('input[type="password"]');
                const hasLoginBtn = !!document.querySelector('button[type="submit"], input[type="submit"]');
                const bodyText = document.body ? document.body.innerText : '';
                const hasLoginText = bodyText.includes('统一认证') || (bodyText.includes('登录') && bodyText.length < 2000);
                return hasPasswordInput || (hasLoginBtn && hasLoginText);
            }""")
            if has_login_form:
                dprint("[DEBUG] 页面内容检测到登录表单，标记需要登录")
                need_login = True
        except Exception:
            pass
    ...
    for i, staff_id in enumerate(STAFF_IDS, 1):
        try:
            ...
            if _is_browser_disconnected(e):
                iprint(f"\n⚠️  抓取员工{i}时浏览器断连，尝试重连...")
                if await browser_manager.reconnect_with_timeout(max_retries=3):
                    page = browser_manager.page
                    # 重连成功，重试当前员工
                    try:
                        if staff_id == MY_ID:
                            result = await fetch_single_staff_v10(page, staff_id, week_before, f"员工{i}(重连后)", i, len(STAFF_IDS))
                        else:
                            result = await fetch_single_staff_v10(page, staff_id, targetweek, f"员工{i}(重连后)", i, len(STAFF_IDS))
                    except Exception:
                        iprint(f"❌ 重连后抓取员工{i}仍然失败")
                        result = {'id': staff_id, 'date': targetweek, 'status': 'error', 'error': str(e)}
                else:
                    iprint("❌ 浏览器重连失败，程序退出")
                    return None
            else:
                iprint(f"❌ 抓取员工{i}异常: {e}")
                result = {'id': staff_id, 'date': targetweek, 'status': 'error', 'error': str(e)}
        ...
        if result['status'] == 'login_required':
            ...
            if await _wait_for_login(page):
                # 登录成功，将当前员工加入重试列表
                staff_date = week_before if staff_id == MY_ID else targetweek
                failed_tasks.append({
                    'staff_id': staff_id,
                    'date': staff_date,
                    'desc': f"员工{i}",
            ...
            try:
                ...
                if _is_browser_disconnected(e):
                    ...
                    if await browser_manager.reconnect_with_timeout(max_retries=3):
                        ...
                        try:
                            result = await fetch_single_staff_v10(
                                page, task['staff_id'], task['date'],
                                task['desc'] + "(重连后重试)", task['index'], task['total']
                            )
                        except Exception:
                            result = {'id': task['staff_id'], 'date': task['date'], 'status': 'error'}
                    else:
                        iprint("❌ 浏览器重连失败，程序退出")
                        return None
                else:
...
async def main_cli():
    ...
    if len(sys.argv) > 1:
        ...
        try:
            ...
            iprint(f"使用命令行参数: 周数={TARGET_WEEK}")
        except ValueError:
            iprint(f"错误：参数「{sys.argv[1]}」不是有效数字！")
            return
    else:
        # 使用配置文件默认值
        TARGET_WEEK = config.target_week
        iprint(f"使用配置文件: 周数={TARGET_WEEK}")
    ...
    if len(sys.argv) > 2:
        ...
        if platform_arg in ["BS", "DS", "DB", "KM"]:
            ...
            iprint(f"使用命令行参数: 平台={LLM_PLATFORM}")
    else:
        # 使用配置文件默认值
        LLM_PLATFORM = config.default_platform
        iprint(f"使用配置文件: 平台={LLM_PLATFORM}")
    
    # 显示最终使用的参数
    iprint(f"\n最终运行参数:")
    iprint(f"   - 目标周: {TARGET_WEEK} ({'本周' if TARGET_WEEK==0 else '上周' if TARGET_WEEK==1 else '上上周'})")
    iprint(f"   - 平台: {LLM_PLATFORM}")
    iprint("="*60 + "\n")
    ...
    iprint("="*60)
    dprint("\n【V10优化版】")
    dprint("  ✓ O1/O2/O3标题严格提取整行内容（包括标记后的所有文字）")
    dprint("  ✓ KR1/KR2/KR3标题严格放入title字段（绝不混入weekly_work）")
    dprint("  ✓ 本人和下属抓取逻辑完全一致")
    dprint("  ✓ 第一部分：本周工作总结（支持多次出现合并）")
    dprint("  ✓ 第二部分：OKR进度（三级表格结构）")
    dprint("  ✓ 第三部分：业务核心指标、主要项目、下周重点工作")
    dprint("  ✓ 新增：首次页面等待5秒")
    dprint("  ✓ 新增：失败页面自动重试机制（最多1次）")
    dprint("  ✓ 【优化】团队周报改为JSON格式（类似个人周报）")
    dprint("  ✓ 【优化】OKR标题去重：去掉重复的 O1/O2/O3/KR1/KR2/KR3 字样，保留一个")
    dprint("  ✓ 【新增】浏览器持久化复用：保存浏览器信息，下次可复用当前浏览器（参考工时脚本实现）")
    ...
    for i, staff_id in enumerate(STAFF_IDS, 1):
        dprint(f"fetch_single_staff_v10参数：{page},{staff_id},{targetweek}")
