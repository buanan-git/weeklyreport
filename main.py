...
function load(){
  fetch('/api/config').then(r=>r.json()).then(c=>{
    ...
    document.getElementById('api_key').value=c.llm_chat?.api_key||platformConfig.api_config?.api_key||'';
  });
}
...
function save(){
  ...
  fetch('/api/save',{
    ...
    body:JSON.stringify((() => {
      ...
      platformUpdate[platform]={type:callType};
      return {
        version:'1.2.0',
        ...
        llm_chat:{api_key:callType==='api'?document.getElementById('api_key').value.trim():''},
        logging:{debug:false}
      };
    })())
